...
- 优先级：P1
- 发起角色：Bridge
- 下一责任方及是否已接单：已关闭；发布验收另走 BR-006
- web_status：source_ready
- app_status：verified
- verification_status：固定 D SHA 已由 `314aedd7c` 合并到 `chat-agent`；精确合并树目标 237/237、App 全量 2860/2860、typecheck、iOS build、Web live health 与两台 Simulator 安装启动通过
- 依赖/阻塞：无本地产品阻塞；远程部署和实体设备属于 BR-006 发布验收
...
- Web/App SHA：规划 `c66761eea`；0026 合入基线 `c0d0ac094`；主体功能 `6dd44b94a`；视觉／Dynamic Type `7df4819a2`；固定最终 SHA `d37d6545d`；主线 merge `314aedd7c`。
...
- 2026-09-15，Bridge：原生 Dynamic Type 暴露固定行高／导航重叠，RED→GREEN 修复后冷启动截图和 accessibility tree 通过；D 分支本地验证完成，BR-022 更新为 `consumer_ready`，等待主线合并树验证。
- 2026-09-15，Bridge：固定最终 SHA `d37d6545d` 由 `314aedd7c` 合并；主线目标、全量、typecheck、iOS build、Web live health 与 Simulator 安装启动通过，BR-022 更新为 `verified`。
