...
## App 开发时保持 Web/API 实际运行

涉及 App 可见行为、API、共享契约或状态同步时，Web/API 是 App 验收环境的一部分。先完成 Web 生产构建，启动构建产物并通过健康检查，再让 Web 浏览器与原生 App 使用同一服务地址、同一账号和同一数据库执行双向回读。仅有源码、mock、路由测试或 Metro 进程不构成跨端验收。

Web/API 或共享契约每次更新后，都要停止旧 Web 进程、重新构建、重新启动并再次检查健康状态，然后重跑受影响的 App 运行时场景。记录 Web SHA、构建日志、进程地址、运行模式、App API base URL 和脱敏账号／记录 ID，避免把旧进程结果归到新源码。
