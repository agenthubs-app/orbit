# Orbit iOS Design Previews

Visual proposals are grouped by feature. Each option keeps its original `exec` identifier when available so feedback can reference the exact image.
...
| `todo-detail-options/` | To-do detail concepts |

Selected references include `selected` in the filename. Revised designs should be added to the same feature folder with a `revision-N` prefix.
