# 关系收件箱面板（Relationship Inbox Panel）落地设计
...
| 字段 | 内容 |
| --- | --- |
| 原始来源 | `repos/orbits/docs/architecture/relationship-inbox-panel.md` |
| 中文镜像 | `knowledge/docs/zh/relationship-inbox-panel.zh.md` |
| 分类 | `architecture` |
| 状态 | `historical` |
| 新鲜度 | `likely-current` |
| 负责人域 | `app` |
...
设计并记录了 /app/** 顶栏右上角的 shell 级 slide-over 面板：单入口两 tab（对话线程 + 提醒/主动提示），复用 chat/message-draft/notifications/proactive 既有契约，新增 createConversationFromDraft 写入路径；文末实现状态显示步骤 0–5 已于 2026-07-09 全部落地。

## 审计依据

已执行完成的一次性落地设计（含拍板决定与实现状态快照）；面板当前行为以 repos/orbits/app/(app)/app/inbox/ 下组件、app/api/chat/relationship-inbox/route.ts 及 tests/pages/app-relationship-inbox-*.test 系列测试为准，发送安全边界（外发全 false、停在本地 staged 预览）仍是有效约束。

## 结构化阅读入口

- 第 1 节：关系收件箱面板（Relationship Inbox Panel）落地设计方案
- 第 2 节：1. 目标与范围
- 第 3 节：2. 复用的现有契约 / 服务 / API
- 第 4 节：3. 信息架构：两类数据分区（关键设计决定）
- 第 5 节：4. 挂载点与组件结构
- 第 6 节：5. 数据入口与 view model
- 第 7 节：6. "发起新对话"如何串联两个服务
- 第 8 节：7. 发送安全边界（必须保留）
- 第 9 节：8. 分步实现计划（建议按 sprint 切）
- 第 10 节：实现状态（2026-07-09：步骤 0–5 全部完成）
- 第 11 节：9. 测试要求
- 第 12 节：10. 已拍板决定（2026-07-09）
...
> 状态：设计蓝图（未开工）。目标是给右上角新增一个 shell 级 slide-over 面板，
> 统一"对话往来"和"提醒信号"两类信息，并把"给人脉发信息"闭环接到真实服务。
