...
| 字段 | 内容 |
| --- | --- |
| 原始来源 | `repos/orbits/features/orbit-ai/FOLLOWUP_CONTEXT_EVALUATION.md` |
| 中文镜像 | `knowledge/docs/zh/orbit-ai-followup-context-evaluation.zh.md` |
| 分类 | `evaluation` |
| 状态 | `current` |
| 新鲜度 | `likely-current` |
| 负责人域 | `orbit-ai` |
...
Sprint 85 跟进上下文解析的评估文档：chat.context 工具计划经会话候选打分（接受阈值 0.7，模糊高分必须 pending）后才调用 generator，十个命名用例覆盖直接命中、失效会话 id、同名歧义、中英文输入和隐私受限等场景。大量篇幅记录失败模式清单——原始来源标签/技术 token/actor 与 ISO 时间戳不得泄漏到主面板、移动端证据必须折叠分层等呈现约束。

## 审计依据

这是能力评估与失败案例分析文档，其失败模式清单是 UI 呈现的审计标准；实际解析与面板行为以 chat-context-artifact-service.ts 和 orbit-ai-followup-context-evaluation 测试为准。

## 结构化阅读入口

- 第 1 节：Orbit AI 跟进上下文评估说明
- 第 2 节：目标
- 第 3 节：设计-评估-分析循环
- 第 4 节：十个命名用例
- 第 5 节：失败案例分析
