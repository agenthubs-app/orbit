...
| 中文镜像 | `knowledge/docs/zh/feature-actions-design.zh.md` |
| 分类 | `feature-design` |
| 状态 | `current` |
| 新鲜度 | `likely-current` |
...
记录 actions feature 的设计边界和 mock-first 实施方向，是模块文档之后的第二层阅读材料。
...
外部操作仍须在「全部安排」（/app/agent/actions）查看详情后确认。没有操作的普通答复不生成空状态卡。最近记录会由服务端压缩为独立的 `userRecordedOutcomes` 上下文交给 planner 和 synthesis。它只能
