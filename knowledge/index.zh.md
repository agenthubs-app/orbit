...
这是 Orbit 项目的结构化知识入口。理解项目时，先读这里，再进入主题知识页和文档库，最后回到代码与测试确认真实行为。

知识库不是代码的替代品。它负责告诉你“该看哪里、当前文档是否可信、哪些内容只是历史背景”。改代码前仍要阅读对应 feature、app route、shared contract 和测试。
...
2. 找文档：读文档库目录，根据 `status` 和 `freshness` 判断它是当前入口、历史材料还是已知过期内容。
3. 修改代码：先读相关知识页和 catalog 条目，再读对应 feature、app route、shared contract 或 harness 代码。
4. 排障：先读 learnings，再定位测试、日志和源码。

## 状态判断

- `current`：可以作为当前阅读入口，但仍要以代码和测试为准。
- `historical` / `superseded`：只解释背景，不直接代表当前实现。
- `verified-current`：最近已经和代码或测试核对过。
- `likely-current`：有来源和关联路径，但动手前要重新确认。
