...
## [2026-08-30] implementation | iOS 行业分布改为环绕标注图

- 用户目标：取消“柱状图/列表加图表”的行业呈现，让所有行业信息围绕图表常驻显示，并在点击扇区或标签后看清当前分组。
- 选定方案：方案一“轨道式引线环”，视觉来源保存在 `docs/designs/orbit-app/network-structure/2026-08-30/01-orbital-callout-halo.png`；另外两份未选方案也保留在同目录，供后续设计追溯。
- 修改摘要：新增纯布局模型 `analysis-pie-orbit-layout.ts` 和原生组件 `AnalysisPieOrbitChart.tsx`；行业维度使用九个环绕标签与 SVG 引线，其他结构维度继续使用原有完整图例；扇区和标签只更新选择，详情导航保留给显式按钮。
- 使用规则：进入 `名片夹 → 人脉分析 → 结构 → 行业`；轻触扇区或环绕标签切换选择，图心同步显示完整行业名与占比，轻触 `查看这个分组` 才打开联系人列表。
- 无障碍与大字：九个行业标签均为可选择按钮并提供完整 VoiceOver 文案；大字体下组件按真实字号增加布局高度和标签宽高，不截断行业名、人数或百分比。独立复核发现并复现了旧公式的 5pt 碰撞后，新增 `1.5 / 1.6 / 2.2 / 3.2` 倍字号矩形防碰撞回归测试，并用 iOS Accessibility Medium 实际渲染复验。
- 文档与证据：更新 `repos/orbit-app/README.md`、原方案设计文档和项目根 `design-qa.md`；最终模拟器截图与同图对照保存在 `repos/orbit-app/.tmp/visual-qa/`。
- 验证方式：布局单测覆盖标准宽度、大字体和完整无障碍文案；全量 iOS 测试 708/708、知识库维护测试 6/6、TypeScript 和 Git diff 检查通过；GitNexus 评为低风险且没有受影响执行流程。

## [2026-09-17] coordination | Web待办核对与并行任务分工

- 用户要求：以本任务为主开发协调入口，提取Web待办、标记已解决项，并按Astra high设计/review、Luna max编码组织独立任务。
- 交付：`docs/development/2026-09-17-web/README.md`覆盖原30项、11组已解决子项、剩余责任域、证据和基线；`SESSION-GUIDE.md`规定文件所有权、隔离工作树、交接和集成流程。原讨论文档与知识库入口新增链接。
- 关键核对：原目录e643572落后于已提交集成29efb4c9；读取成本/账号/环境切换继续归既有数据任务。Pipeline删除已被后续保留决定替代；App/PhoneWeb证据不能代替Next Web。
- 调度：创建W1资料、W2名片、W3活动、W4 AI分析四个Astra high任务，首阶段提供具体设计包，批准编码批次后使用Luna max。首次创建和实际就绪状态见总表，不把创建请求当编码完成。
- 验证：源码/图谱/测试内容/历史报告审计；30个原ID完整且不重复；文档diff-check。未在主任务重跑全量测试或云端验收，领域W3报告的23/23只作为该工作树基线定向结果。
