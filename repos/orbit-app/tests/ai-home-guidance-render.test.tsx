...
const summary = (questionSignals = { urgentTask: false, relationshipTask: false, preparation: false }) => ({
  taskMode: "summary",
  date: todayDate,
  timeZone: "Asia/Tokyo",
...
test.beforeEach(() => { today = summary(); todayKind = "success"; conversation = emptyAiConversationPayload; });
...
test("relationship work offers follow-up planning alongside activity discovery", () => {
  today = summary({ urgentTask: false, relationshipTask: true, preparation: false });
  const html = renderToHtml(<AiScreen />);
...
test("urgent open work takes priority over relationship suggestions", () => {
  today = summary({ urgentTask: true, relationshipTask: true, preparation: false });
  const html = renderToHtml(<AiScreen />);
  assert.match(html, /今天先处理哪些事？/u);
...
test("upcoming scheduled events offer preparation without claiming a registration", () => {
  today = summary({ urgentTask: false, relationshipTask: false, preparation: true });
  const html = renderToHtml(<AiScreen />);
