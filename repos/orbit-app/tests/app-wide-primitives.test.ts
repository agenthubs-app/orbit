...
const unexpectedNotificationAction = () => { throw new Error("Layout checks must not change notification settings"); };
export const setPushNotificationsOptIn = unexpectedNotificationAction;
export const revokeNotificationDevice = unexpectedNotificationAction;
export const revokeRegisteredPushDevice = unexpectedNotificationAction;
...
createRoot(document.getElementById("root")).render(<Fixture />);`, resolveDir: process.cwd(), loader: "tsx" },
    ...
    plugins: [{ name: "primitives-boundaries", setup(plugin) {
      ...
      plugin.onResolve({ filter: /^(expo-router|@expo\/vector-icons|react-native-safe-area-context)$|\/(AuthSessionProvider|ApiBaseUrlProvider|useOrbitApiClient|native-notifications|push-device-session)$/ }, () => ({ path: "fixture", namespace: "primitives-test" }));
