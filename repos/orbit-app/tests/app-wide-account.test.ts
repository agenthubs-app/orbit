...
export const useOrbitApiClient = () => ({ post: async (path, options) => record("POST")({ path, body: options?.body }), put: async (path, options) => record("PUT")({ path, body: options?.body }) });
...
createRoot(document.getElementById("root")).render(<Screen mode={params.get("mode") || "login"} surface={params.get("surface") || "dashboard"} />);`, resolveDir: process.cwd(), loader: "tsx" },
    ...
    plugins: [{ name: "account-boundaries", setup(plugin) {
      ...
      plugin.onResolve({ filter: /^(expo-router|@expo\/vector-icons|react-native-safe-area-context|expo-image-picker|expo-document-picker)$|\/(AuthSessionProvider|useApiResource|useOrbitApiClient|ApiBaseUrlProvider)$|\/api\/client$/ }, () => ({ path: "fixture", namespace: "account-test" }));
...
for (const theme of ["light", "dark"] as const) {
  ...
  for (const mode of ["login", "signup"] as const) {
    test(`${theme}: ${mode} keeps editable credentials after a failed attempt`, async t => {
      ...
      await submit.click(); assert.equal(await page.getByRole("button", { name: mode === "login" ? "登录中..." : "创建中...", exact: true }).isDisabled(), true);
...
test("forgot mode and signed-out profile/permission/account preserve public boundaries", async t => {
  ...
  assert.equal(await forgot.getByRole("textbox").count(), 0);
  const returnAction = forgot.getByRole("button", { name: /返回登录/ }); await fits(returnAction, 50); await returnAction.click();
