...
for (const [width, scheme] of [[390, "light"], [1280, "dark"]] as const) test(`review layout fits ${width}px ${scheme} and retains input on theme change`, async t => {
  ...
  assert.equal(await p.locator("#root > div").evaluate(element => getComputedStyle(element).backgroundColor), scheme === "dark" ? "rgb(34, 38, 46)" : "rgb(255, 255, 255)");
  if (process.env.TASK3_VISUAL === "1") console.log(`TASK3_IMAGE_${width}:` + (await p.screenshot({ fullPage: true })).toString("base64"));
  await p.emulateMedia({ colorScheme: scheme === "light" ? "dark" : "light" }); await p.waitForFunction(expected => getComputedStyle(document.querySelector("#root > div")!).backgroundColor === expected, scheme === "light" ? "rgb(34, 38, 46)" : "rgb(255, 255, 255)"); assert.equal(await p.getByLabel("备注", { exact: true }).inputValue(), "Layout draft");
});
