...
const relationshipConversation = { conversationId: "thread-one", contactId: "person-one", participantAccountIds: ["reader", "account:lin"], participantDisplayNames: { reader: "我", "account:lin": "林悦" }, qualificationVersion: "qv:style", status: "active", createdAt: "2026-09-07T00:00:00Z", updatedAt: "2026-09-07T01:00:00Z", unreadCount: 0, messages: [relationshipMessage] };
...
function dataFor(path) {
  if (path.startsWith("/api/task-suggestions/page?")) return {actorId:"reader",scope:"relationship",items:[],total:0,nextCursor:null,hasMore:false,asOf:"2026-09-25T00:00:00Z"};
  if (path === "/api/relationship-communication/unread-summary") return { actorId: "reader", unreadTotal: 0, refreshedAt: relationshipConversation.updatedAt };
  if (path.startsWith("/api/relationship-communication/conversation-summaries")) { const { messages, ...summary } = relationshipConversation; return { actorId: "reader", items: state.empty ? [] : [{ ...summary, lastMessage: { messageId: relationshipMessage.messageId, senderAccountId: relationshipMessage.senderAccountId, sentAt: relationshipMessage.sentAt, bodyPreview: relationshipMessage.body } }], hasMore: false, nextCursor: null, asOf: relationshipConversation.updatedAt }; }
  if (path.startsWith("/api/relationship-communication/conversations/thread-one/messages?")) { const { messages, unreadCount, ...identity } = relationshipConversation; return { actorId: "reader", conversation: identity, items: state.empty ? [] : messages, hasMore: false, nextCursor: null, newestCursor: state.empty ? null : "newest", direction: "older", asOf: relationshipConversation.updatedAt }; }
  if (screen === "ai" && path.includes("/sessions")) return { ...aiSessionListPayload, items: state.empty ? [] : [{ ...aiSessionListPayload.items[0], id: "session-one", title: "合作计划", firstUserText: "讨论合作时间", organization: { ...aiSessionListPayload.items[0].organization, customTitle: "讨论合作时间", pinned: true } }] };
