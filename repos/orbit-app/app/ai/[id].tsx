...
export default function AiConversationRoute() {
  ...
  const intent = useMemo(() => {
    ...
    return { submitted: false, owner: origin ? JSON.stringify([origin.actorId, origin.baseUrl]) : null as string | null };
  }, [intentKey]);
  ...
  return enabled ? <ProtectedConversation key={scope.key} scopeKey={scope.key} isScopeCurrent={isScopeCurrent} claimInitialPrompt={claimInitialPrompt} allowInitialPrompt={allowInitialPrompt} journal={journal} /> : null;
}
