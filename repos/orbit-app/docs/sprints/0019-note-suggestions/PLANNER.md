...
**Plan revision:** 2（记录已批准的 IORBIT 模板入口；执行范围仍待补齐审阅）。**模式:** existing-codebase / single-generator；运行状态只在[登记表](../README.md)；具体运行状态以登记表为准。
**原需求:** R-13建议和R-14后期收口；依赖0018已交付原文／关联。
**目标:** 从笔记模板进入 IORBIT，用户发送后基于笔记版本生成可确认的事项建议，用户接受后只创建一次，并完成含笔记的最终验收。
...
用户已认可以下流程，不重复确认相同产品决定：

1. 笔记页提供“从这篇笔记整理待办”的模板入口，不在笔记页独立调用 AI 提取待办。
2. 点击后携带当前笔记引用进入 IORBIT，预填可编辑的请求；跳转、预填和读取笔记都不自动调用模型或创建事项。
3. 用户发送后才生成建议；建议仍关联来源笔记 ID、版本和相关人脉，保留权限校验与日期歧义确认。
4. 用户确认建议后才创建事项；沿用服务端接受幂等、原文变更重新校验和失败保留规则。

这项批准不等于批准旧笔记迁移或启动实现。实施前须将 IORBIT 的笔记引用、建议展示与确认写入协议、涉及的 AI 页面／服务端文件和逐步交互测试补入执行范围并审阅；下方现有文件白名单不自动扩张。笔记列表与编辑入口的位置仍由 0018 的设计确定。
...
**不做：** 笔记页直接调用 AI 生成建议、仅跳转 IORBIT 就自动发送、读笔记自动创建事项、多人关联就复制任务、无声推断歧义日期、自动通知／外部日历写入。

## 验收契约

| SC | 可观察结果 | 最小必要证据 |
| --- | --- | --- |
| SC-0019-01 | 笔记入口只用模板与笔记引用预填 IORBIT，用户发送后才生成建议；建议记录来源笔记ID／版本和相关人脉，日期歧义需用户确认。 | 模板跳转／预填／发送前零模型请求、显式发送后生成、真实引用契约及明确／歧义日期交互。 |
