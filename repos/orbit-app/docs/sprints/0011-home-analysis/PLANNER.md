...
**Plan revision:** 2（补入首页活动位置、待办上限与保留 Pipeline 的决定）。**模式:** existing-codebase / single-generator。运行状态只在登记表。
...
- 首页增加“记笔记”小入口，位于“新建待办”之后，实际笔记创建路由由 0018 交付后接通；不为 0011 制造依赖 0018 的循环，也不先交付可点击的空路由。
- 源码另有顶部“联系跟进”快捷按钮，和被替换的内容区块是两处。用户同时说不需要首页跟进入口、又指定记笔记位于新建待办与联系跟进之间；该快捷按钮保留／移除已单独询问，未答复前不擅自固定最终快捷项列表，其余明确决定照常记录。
...
缺剩余首页快捷项澄清或 B7 版本／目标契约时不启动依赖部分；不重复询问已确定的活动位置、待办上限或 Pipeline。不能把 client 当前时间伪装为生成时间，也不继续整份资料覆盖式目标保存。
