...
- run：run-01；Task 4 验收日期：2026-09-16（Asia/Tokyo）。
- 分支：`codex/sprint-0032-hybrid-sync-foundation`。
- Sprint 基线：`1129d72d3`；Task 4 开始和被验收的最后功能 HEAD：`0c1cf70bb8b7004d11cef972a9a76f2048e154e8`；初始工作树干净。
...
- 本线最终 SHA：报告提交后由 Task 4 交接提供固定完整 SHA；报告不追填自身 commit。
- `chat-agent` 精确 merge SHA／合并树复验：**等待协调者**。
- push／部署：未执行。
- 原始证据：本工作树忽略目录 `.superpowers/sdd/PLANNER/task-4-*`。详细交接为同目录 `task-4-report.md`。
...
| Task 4 文档提交 | 实测验收与登记；无产品或测试源码修改 | 全部 |
...
| SC | 本线结果 | 实际证据与边界 |
| --- | --- | --- |
| SC-0032-01 | pass | Web authority/contract 8/8；App contract/schema/domain 同步检查在 90/90 定向集中通过。Task 1 RED→GREEN 证明缺失契约及不透明 revision 被错误归一化时失败。 |
...
| SC-0032-03 | pass | lifecycle/repository/auth 定向集通过；Simulator seed 8/8、重启／清理／隔离 35/35。server/actor 拒读，workspace 切换拒绝旧 handle，第二 workspace 与第二 actor 无记录/cursor/outbox/snapshot，登出密钥及主文件/sidecar 删除，重新打开第一 actor 仍不可恢复。 |
...
| SC-0032-05 | pass | App 139/139 定向及直接消费者测试、两端 typecheck；native snapshot 重启回读通过。原始产品包原生启动并显示登录页，已目视检查截图。四域页面、hooks、view-models、文案相对基线 diff 为空，无实体镜像 reader 接入或 offline write 文案。 |

## 本次最小验证

所有命令针对功能 HEAD `0c1cf70bb`，退出码均为 0，除下节明确保留的初始环境失败。
...
最终文档暂存检查：`git diff --check` 与 `git diff --cached --check` 通过；严格路径审查仅本报告和 Sprint 登记表。`gitnexus_detect_changes(scope="staged")` 返回 2 个文档文件、2 个 README section、0 个受影响执行流程、LOW；没有产品符号变化。原始结果见 `task-4-staged-detect.json`。
...
- Task 4 唯一提交内容为本报告与 `docs/sprints/README.md`。原始证据、生成 iOS/Pods、harness、DerivedData 与安全学习条目均不提交。
