...
| [0084](0084-profile-page-hierarchy-audit/GOAL.md) | 资料类页面层级：全面截图审核 → 设计案 → 统一 `ProfilePagePrimitives` 四类元素规格 | TODO 第 2 条。含用户批准门（设计案）；审核范围含设置／账号／活动详情等同结构页面。档位 App H。基线 `12a9f9653`，Planner SHA 81db145c。**2026-09-19 用户批准设计案**（https://claude.ai/artifact/RPmcSJo9GBTkucNoBPUj97），批准门已通过。功能 `3425a7d05`／合并 `1e302a42f`。审核对 19 处 `sectionTitle` 逐一核对，命中 8 页落在 3 个文件；活动／待办／日程详情逐条排除（其 section 标题领起正文与日期分组卡片，下面没有标签值行）。规格落在新 `rowRoleStyles`，不动被 65 个文件引用的 `textStyles.section`。全量 3509/3509、0 fail、201s。Simulator（iPhone 17 Pro，重建后）抽查设置页与编辑资料页：分组标题 12/600 灰、字段标签 13/500、静态值 16/400，四角色在原生端同样分开 | completed |
| [0085](0085-iorbit-entity-read-show-write/GOAL.md) | IORBIT 五实体读／展示／写：实体小卡片、草稿卡确认状态机、五种创建、去掉"AI 运行依据" | TODO 第 4 条。含用户批准门（设计案须回答卡片规格／状态机／写入接口／prompt vs workflow）；设计案若超一个 Sprint 则拆 0086。档位 orbits H / App H。基线 `12a9f9653`，Planner SHA f45ef0b8。**2026-09-19 用户批准设计案**（https://claude.ai/artifact/De2NsaKmsAe7JaSRJSvqHc），批准门已通过；Planner 里“若超一个 Sprint 则拆 0086”因 0086 已被占用，实际拆到 0093。**根因不是 prompt**：planner 每轮只能选一个 intent，"根据笔记整理待办"把机会花在读上，且此时还没看到笔记内容；草稿改到工具返回后再问，触发用确定性正则。真实库+真实模型走通待办／笔记／日程（64→65、回读一致、重复确认 404）；人脉被既有护栏 `live-agent-runtime.ts:750` 拦在模型之前，活动被活动服务 source note 前置条件拒绝（失败可见、可重试），两者拆 0093。App 全量 3525/3525。orbits 全量受 Neon 云库额度耗尽污染，已用基线对照区分。**2026-09-19 更正**：SC-0085-02 初版误记 pass——引用的"五种 kind 屏幕测试"测的是**草稿卡**（创建），而该 SC 要的是**展示**卡片；展示那一半未做，对话里仍是既有的四个各异面板 + 笔记无卡片。用户在 Simulator 上试出来的，不是自查发现。展示另立 0094 | partial |
...
| [0087](0087-never-synced-empty-state/GOAL.md) | 没同步过就不要说“暂无”：镜像快照新增 `unsynced` 初始态，所有 `useSyncedCollection` 消费页在未同步时显示加载态而非空态 | 复核第 1 条。根因：`emptySnapshot()` 初始就报 `local-ready`，`mirrorTaskListSource` 同时算出 `loading` 与空态。与 0078「失败要可见」同族。档位 App H。基线 `e6153d1ee`，Planner SHA c5b43c40。`unsynced` 立在快照层；未同步时 `canonical` 为 null，空态因此没有渲染条件。枚举确认全仓只有待办列表消费 `useSyncedCollection`（native/Web 同经 `mirrorTaskListSource`），规则立在共享层供后续页面继承。修正两处自身错误：`syncing` 也是下拉刷新的状态，改用 `lastSyncedAt` 作证据；首次同步失败原会同时显示错误态与空态。无新增 i18n key。App 全量 3534/3534 | completed |
| [0088](0088-relationship-dashboard-first-paint/GOAL.md) | 关系仪表盘首屏：定位 `12000ms timeout exceeded` 出处、量六个 `useApiResource` 耗时、分块首屏、超时落到可重试错误态 | 复核第 2 条（本轮唯一 console 错误）。先量后改；App 内无 12000 常量，出处待定位。档位 App L（度量指向服务端则升 orbits H）。基线 `ededaa6ac`，Planner SHA bb979660。**15 秒白屏与 `12000ms timeout exceeded` 都不复现**：首屏 245–256ms（热），console 无任何超时错误；两仓库唯一的 `12_000` 是 `agent-tools/registry.ts:187` 的 agent 工具超时，不在本路径。与 0091／0092 同源（复核时磁盘仅剩 226MB）。**六个接口是并发的**，+130ms 同时发起、落地相差 <20ms、各 57–69ms——Planner 设想的并发上限／串行依赖／单接口慢三种可能实测均不成立，因此未碰服务端。**真实问题是首屏挂在 `aggregate` 一个资源上**：一个读不回来整页白。改为任一资源落地即渲染，覆盖度卡自述状态（正在读／超时可重试／不可用可重试），**绝不画成 0%**（0087 那条规则：没读到不等于没有）；上限复用 0092 的 8s。浏览器只挂住覆盖度请求：3s 其余区块可见且卡片说在读，10s 变超时可重试，console 无错误——改前这两个时刻都是白屏。App 3572/3572 | completed |
...
| [0097](0097-orbits-suite-signal/GOAL.md) | orbits 全量 48 条既有失败：要么修好，要么明确分类，使一次全量即可判断回归 | 0095／0093 报告记下的遗留。**已做**：48 条全部归群（19 个文件，7 群，表在 PLANNER）；修好 7 条，48→41，零新增，通过数 4019→4026。A 群=测试用 `split(";")` 切 schema，被 `migrations.ts:47` 注释里的分号切断（生产是整段执行）；C 群=`apiKey: null` 语义是"未指定、回落环境变量"，三条"无 key 应失败"用例在配了 key 的机器上真去调了提供方、结论与用例名相反，现改为调用前后清掉 key；F 群=六个契约文件从未进 `index.ts`，补出口时发现 `offline-policy` 与 `universal-read` 独立定义了三个完全相同的类型，已去重。另修一条 0094 留下的过期 prompt 断言。**撤回一次改动**：把 `projectPublicProfile` 从契约移到 `shared/domain` 后，App 三条 `domain-sync` 用例变红——domain 对客户端只开放 `industries.ts`／`language.ts`，契约又禁止运行时代码，**这份共享逻辑当前没有被认可的去处**，属架构决定，已整体退回并留红。**SC-0097-03 未达成**：仍有 41 条红且分类只在文档里，一次全量依然不能独立判断回归。交接建议：先决定 D 群（12 条同属"运行时证据"机制，若需人工执行则应移出 `npm test` 默认集合） | partial |
| [0096](0096-private-event-visibility/GOAL.md) | 私有活动的可见位置：让聊天里确认创建的活动在 App 里能打开，且不因此进入任何对外列表 | 0093 缺口。实测：记录已建好且 `GET /api/events/<id>` 可读，但 App 详情页请求 `/api/events/public/<id>` 404——App 七个活动界面读的都是 canonical 公共目录。**进入条件：需用户先就可见性语义做决定**——A 私有活动走自己的详情路径（改动小、可逆，代价是活动有两种详情页）／B 创建后需发布（语义统一，但要处理主办方身份、报名、公共编码）。判断：无论选哪条，必须有"另一账号读不到、对外列表不多出这条"的反例。档位 orbits M。基线 0093 合并后 | planned |
| [0095](0095-entity-artifact-recovery-beyond-contacts/GOAL.md) | 会话恢复保留五种实体 artifact：放开 `otherKinds` 过滤、让落盘保留真实 kind、把 `contactHref` 的 refinement 从人脉专用推广成 kind→前缀→路径 表（保留反例） | 0094 未完成的那一半，根因已定位到服务端。判断：refinement 是把证据 id 与详情链接绑定的安全校验，要推广不要放宽；条目/字节预算不变。档位 orbits H（持久化 + 跨端契约）。基线 0094 合并后。**已做**：过滤名单从"丢哪些"改成"哪些带记录"，映射保留真实 kind，元数据白名单补上每张卡显示的那一行（否则待办恢复回来没有截止日期、日程没有开始时间）；条目/字节预算未动。链接规则**按 kind 推广且更严**：人脉仍必须有 `contact:review:<id>` 绑定，`data_query` 条目本就没有 action 只能以自身 id 为准，而任何指向别的记录的 href 一律作废——原实现只校验那一条 review action，站外 href 挂在别的 action 上会漏过去。**真跑才发现的两个 0094 遗留缺陷**：日程详情是三个路由不是一个（`/schedule/<id>` 根本不是路由，点下去落首页），改为按条目自身 `kind` 取路由段，说不清的不给链接；时刻原来按标签名格式化，中文「开始」漏掉打印原始 ISO 串，改为按值判断。五种实体各问一句、各重开会话、各点进详情页均通过；App 3556/3556，orbits 48 fail 对照 HEAD 基线 49 fail 无新增。`gitnexus_detect_changes()` 因 MCP 断连未跑成，改人工核对范围 | completed |
| [0094](0094-iorbit-entity-display-cards/GOAL.md) | 对话里五种实体的统一小卡片：一个卡片组件 + 五种 kind 映射，整卡可点进详情；正文不再复述卡片字段；补上笔记卡片 | 0085 设计案「展示」那一半，0085 未做。实测：人脉数据结构化且卡片出得来，但正文把同一批人又列一遍（synthesis 指令要求"point out the strongest matches by name"）。判断：只做一个卡片组件，不再加第五个面板；正文去重在服务端指令收口，不在客户端裁剪。档位 App H / orbits L。基线 0093 合并后。**已做**：统一卡片 view-model + 组件（夹具为五种实体真实抓包，7/7），人脉真实走通（短判断 + 8 张统一卡在首屏），synthesis 指令改为不复述卡片内容（改前整屏列人名分数把卡片挤出屏幕）。**未做**：待办／日程／笔记／活动在"可靠发送"后仍无卡片——根因在服务端 `orbit-agent-chat-session-artifact-reader.ts:10` 的 `otherKinds` 过滤名单把非人脉 artifact 全丢弃，会话恢复 `turns=0`；放宽它要动 `displayItem` 里"contactHref 必须由 `contact-recommendation:` id 推导"这条带安全性质的跨端 refinement，另立 sprint | partial |
| [0093](0093-event-draft-write/GOAL.md) | 活动草稿写入：查清 `A source note is required…` 是产品规则还是 mock 遗留，让活动可从草稿卡确认创建或在确认前告知 | 0085 拆分。**2026-09-19 用户决定放弃"通过 AI 聊天添加人脉"**，人脉继续走既有名片／二维码／手动录入流程，相关代码已移除而非留着不用。档位 orbits L。基线 0085 合并后。**SC-0093-01 结论：是产品规则，不是 mock 遗留**——mock／hybrid／live 三个实现都强制，`in the mock` 只是当年只有 mock 时的措辞；契约里 sourceNote 记录"为什么纳入 Orbit"，且没有任何东西校验其内容。做法：模型为活动草稿产出一句纳入理由，卡片作为可编辑行显示（确认前就能改），模型没给时退回"由 IORBIT 会话 X 草稿 Y 经用户确认创建"这句可核对的出处。**真跑发现两个问题**：`slugFromTitle` 只保留 `[a-z0-9]`，中文标题 slug 成空串，每个手动创建的活动 id 都是 `event:live-record:` 互相覆盖（三份副本合成一个 helper，空 slug 回退标题摘要）；`as unknown as EventCreatePort` 把 port 缺字段的契约漂移藏住了，已去掉强制转换。顺手删掉两处 prompt 里已废弃的 contact 草稿指引，并把草稿卡的日程链接改为 `/schedule/personal/<id>`。真实库：确认前不写、确认后 16→17、重复确认 404 且不重复写、两个不同标题得到不同 id。App 3557/3557，orbits 48 fail 对照基线 49 无新增。**缺口另立 0096**：建出来的活动在 App 里打不开——App 七个活动界面读的都是 canonical 公共目录，而这是未发布的私有活动；该走私有详情路径还是走"创建后发布"是产品决定，未在本 Sprint 内替用户选 | completed |
| [0092](0092-iorbit-home-loading-states/GOAL.md) | IORBIT 首页两块区域：“正在读取最近会话”／“正在核对下一步”加载态加上限，三终态（有内容／确实为空／出错可重试）可区分 | 复核第 6 条。两块属不同子系统，需分别度量；与 0078／0087 同族的第三面——加载态必须有终点。档位 App L。基线 `ededaa6ac`，Planner SHA bb4b71f7。**"一直转圈"复现不出来**：最近会话 267ms、下一步 320ms 即清空，与 0091 同源（复核时磁盘仅剩 226MB）。因此**没有做任何提速**——这里没有等待可省；做的是 Planner 判断 2「加载态必须有终点」。来源与耗时：最近会话 = `/api/ai/conversations` 110ms + `/api/ai/conversations/sessions` 105ms；下一步 = `/api/today` 134ms。新增 `useLoadingDeadline`，上限取 8s（与 `DEFAULT_INVALIDATION_TIMEOUT_MS` 一致，不另造数字），重试重置计时；计时器单独导出以便在只有静态渲染的本仓库里被真实测到。三终态 + 逾期共四种情形、两块区域各有断言，并断言逾期区域不得同时声称"确实没有"。浏览器挂住请求验证：3s 仍在读、9.5s 变为超时 + 重试。前后热跑 267→263ms／320→318ms。App 3567/3567。**记一笔**：每次重新 export 后首跑 1920/2404ms 是构建缓存，非回归 | completed |
