...
后续 Phase A 量尺（0070，completed）、B 止血（0071 completed，0072 completed，0073 running，0074）、C 收口扩面（0075–0076）、D Web 本地优先（0077–0078）、
...
| [0073](0073-search-index-alignment/GOAL.md) | 搜索路径索引对齐：pg_trgm GIN 让 `search_text ilike` 走索引，删从未使用的 tsvector 索引；不改查询、不改语义 | 依赖 0070（completed）。基线 `26423d9b9`，Planner SHA 9d4f24ac。结果逐条一致为硬断言；2 字中文查询不走索引如实记录。档位 M，orbits 全量一次。只用本机 PG | running |
