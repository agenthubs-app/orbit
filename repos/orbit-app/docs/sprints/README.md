...
| [0094](0094-iorbit-entity-display-cards/GOAL.md) | 对话里五种实体的统一小卡片：一个卡片组件 + 五种 kind 映射，整卡可点进详情；正文不再复述卡片字段；补上笔记卡片 | 0085 设计案「展示」那一半，0085 未做。实测：人脉数据结构化且卡片出得来，但正文把同一批人又列一遍（synthesis 指令要求"point out the strongest matches by name"）。判断：只做一个卡片组件，不再加第五个面板；正文去重在服务端指令收口，不在客户端裁剪。档位 App H / orbits L。基线 0093 合并后 | planned |
