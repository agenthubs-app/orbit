...
| [0095](0095-entity-artifact-recovery-beyond-contacts/GOAL.md) | 会话恢复保留五种实体 artifact：放开 `otherKinds` 过滤、让落盘保留真实 kind、把 `contactHref` 的 refinement 从人脉专用推广成 kind→前缀→路径 表（保留反例） | 0094 未完成的那一半，根因已定位到服务端。判断：refinement 是把证据 id 与详情链接绑定的安全校验，要推广不要放宽；条目/字节预算不变。档位 orbits H（持久化 + 跨端契约）。基线 0094 合并后 | planned |
