...
| F — cross-client verification | 本提交 | 本报告、BR-021 与本地验证证据；不把缺少环境的真实账号/provider/migration apply 写成通过。 |
...
这些依赖缺失不影响 A–E 的源码和本地回归结论；补齐环境后按 BR-021 的关闭条件追加证据，不需要重做 authority 设计。
