...
3. 确认 Planner 已获适用批准，依赖／环境／对象／授权就绪，全局非 PAUSED。缺前置则标受阻，不消耗 Generator run，不尝试假接口；这不阻止授权范围内的独立调查、待审方案完善或其他已就绪 Sprint，整体停止须满足第 11 节。
...
继续用户授权范围内的独立工作，不能据此直接结束整体任务。实现只做该 Planner 范围，
...
用户已要求完成 0022 设计后按减少返工的顺序逐步实施，随后要求离线期间独立推进、不再提问。第 6 节关于 0020／0021“当前只编制”的执行指令限制已被本次明确指令更新；其书面计划审阅、依赖、精确写入与其他独立门槛没有被取消。已确认决定直接复用，无需再次确认。缺门槛时继续不需新决定的源码核查、设计补充和就绪准备；不把离线或沉默当成批准。
...
单个 Sprint 不满足启动条件、完成报告或按规则结束为 failed／blocked，只结束该 Sprint 的对应步骤或 run，不自动结束用户授权的整体任务。原单次 Generator、文件锁、失败重试上限和审批门槛保持不变；不得为继续工作重开已结束的 run 或扩大实现白名单。
