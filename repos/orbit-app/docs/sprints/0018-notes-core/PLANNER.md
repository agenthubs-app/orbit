...
**Plan revision:** 2（记录首页记笔记入口；旧内容迁移仍待审阅）。**模式:** existing-codebase / single-generator；运行状态只在[登记表](../README.md)；具体运行状态以登记表为准。
...
首页快捷区增加“记笔记”小入口，放在“新建待办”之后；用户描述为新建待办与联系跟进之间。首页另有联系跟进内容区块与同名快捷按钮，内容区块已由 0011 确定替换成活动；快捷按钮是否也删除待单独澄清，不将两处混为一处。
