...
## 后续局部修复（2026-09-13 15:45 JST）

普通字号待办长标题已在当前登录会话中修复并原生复验：同一失败样本先复现 32pt 裁切，标题固定高度改为最小高度后达到 64pt，返回列表重开仍通过既有尺寸脚本，逐张截图确认全文可见。只改 App 一处布局约束，不改 RN 依赖、字体缩放、原文或保存方式；未写真实业务数据。当前局部版本为 `5b454f73b` 加该改动，详见[2026-09-13 执行记录第十四节](../../verification/2026-09-13-app-connectivity.md#14-r-12普通字号待办长标题)。

下方保留 9 月 12 日的失败和登录状态历史，不将它们当作当前会话状态。普通长标题这一项已获新证据；运行中字号热切换、其他非空流程与整包原生验收仍未通过，因此总体 blocked 不变。

## 当时结论（2026-09-12 22:49 JST）
