...
App 随后为报名问卷启用只走网络的读取策略，失败时保留本页草稿但禁止写入，
不再以旧快照表示本次读取成功；其他内容页的默认缓存不变。15:09 原生问卷仍
500（请求 `04e5d959-ed8d-4ad8-a8ea-4954be531095`），页面已显示错误和重读入口，
没有表单或提交按钮。这修复了 App 的失败呈现，不是问卷服务恢复。
