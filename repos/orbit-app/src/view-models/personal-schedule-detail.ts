...
export function personalScheduleDetail(item: PersonalScheduleContract, fallbackZone: string) {
  ...
  return { id: item.id, updatedAt: item.updatedAt, title: item.title, zone, date: start.date, startTime: start.time, endDate: end?.date, endTime: end?.time, allDay: item.allDay === true,
