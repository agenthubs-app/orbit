...
export function ProfilePublicView({ profile }: { profile: PublicProfileProjectionContract }) {
  ...
  return <View style={styles.stack}>
    ...
    {profile.bio ? <ProfileSection title={locale.t("profile.currentWork")}><View style={styles.bodyRow}><Text style={styles.body}>{locale.t.literal(profile.bio)}</Text></View></ProfileSection> : null}
...
const useStyles = createThemedStyles(colors => StyleSheet.create({
  stack: { alignSelf: "stretch", gap: 24, width: "100%" },
  identity: { alignItems: "center", flexDirection: "row", gap: 16, paddingTop: 8 },
  identityLarge: { alignItems: "flex-start", flexDirection: "column" },
  ...
  avatarText: { color: colors.onAccent, fontSize: 30, fontWeight: "800" },
  name: { color: colors.ink, fontSize: 24, fontWeight: "900", letterSpacing: -0.4 },
  meta: { color: colors.text3, fontSize: 14 },
  secondary: { color: colors.text4, fontSize: 13 },
  headline: { color: colors.ink, fontSize: 15, fontWeight: "700", marginTop: 4 },
  bodyRow: { alignSelf: "stretch", flexDirection: "row" },
  body: { color: colors.ink, flex: 1, flexWrap: "wrap", fontSize: 15, minWidth: 0, paddingVertical: 13 },
}));
