...
export function NotesScreen({ actorId, scopeKey: _scopeKey }: { actorId: string; scopeKey: string }) {
  ...
  const canonical = notesFromPayload({ notes: state.records.map((record) => record.payload) }, actorId, locale.language);
  const notes = useMemo(() => canonical?.filter((note) => {
    const matchesAssociation = contactId
      ? note.contactIds.includes(contactId)
      : filter === "contacts" ? note.contactIds.length > 0
        : filter === "events" ? note.eventIds.length > 0
          : filter === "unlinked" ? note.contactIds.length === 0 && note.eventIds.length === 0
            : true;
    if (!matchesAssociation) return false;
    const needle = debouncedQuery.toLocaleLowerCase();
    return !needle || `${note.title}\n${note.body}`.toLocaleLowerCase().includes(needle);
  }) ?? null, [canonical, contactId, debouncedQuery, filter]);
  useEffect(() => {
    if (state.status !== "fresh" || contactId) return;
    void retireSnapshot(baseUrl, actorId, notesSearchPath({ association: filter, q: debouncedQuery, limit: 20 }));
  }, [actorId, baseUrl, contactId, debouncedQuery, filter, state.status]);
  ...
  return <AppScreen title={locale.t("notes.myNotes")} backAccessibilityLabel={locale.t("common.backToNamed", { name: locale.t("nav.home") })} backLabel={locale.t("nav.home")} refreshControl={<RefreshControl refreshing={state.status === "syncing"} onRefresh={() => { void state.refresh(); }} />} headerActions={
    ...
    <View style={styles.hero}><Text maxFontSizeMultiplier={2} style={styles.heroTitle}>{locale.t("notes.title")}</Text><Text maxFontSizeMultiplier={2} style={styles.heroCount}>{canonical?.length ?? 0}</Text></View>
    <Text accessibilityLiveRegion="polite" maxFontSizeMultiplier={2} style={styles.syncStatus}>{locale.t(`sync.${state.status === "local-ready" ? "localReady" : state.status}`)}{state.lastSyncedAt ? ` · ${locale.t("sync.lastSynced", { time: new Date(state.lastSyncedAt).toLocaleString() })}` : ""}</Text>
    ...
    {canonical === null ? <ErrorState message={locale.t("notes.invalidPayload")} /> : null}
    {notes?.length === 0 && state.status === "fresh" ? <EmptyState title={locale.t(query ? "notes.emptySearch" : contactId ? "notes.emptyLinked" : "notes.empty")} message={locale.t(query ? "notes.emptySearchBody" : "notes.emptyBody")} /> : null}
...
const useStyles = createThemedStyles((colors) => StyleSheet.create({
  ...
  syncStatus: { color: colors.text3, fontSize: typography.caption },
