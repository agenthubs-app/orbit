...
export function EditNoteScreen({ actorId, draftServer = "local", noteId, scopeKey, isScopeCurrent = () => true }: {
  ...
  useEffect(() => {
    ...
    void noteDraftStorage.load({ accountId: actorId, server: draftServer, noteId }).then((stored) => {
      ...
      setTitle(stored.title); setDraft(stored.body); setSelectedIds([...stored.manualContactIds]); setMentions([...stored.mentions]); setEventIds([...stored.eventIds]); setDirty(true); setDraftStatus("已恢复草稿");
    });
  }, [actorId, current, draftServer, noteId]);
  useEffect(() => {
    if (!dirty) return;
  ...
  }, [actorId, dirty, draft, draftServer, eventIds, mentions, noteId, selectedIds, title]);
  ...
  async function save() {
    ...
    const request = buildRichNoteUpdateRequest({ title, body: draft, manualContactIds: selectedIds, mentions, eventIds }, current.version, mutationKey.current);
    ...
    const note = result.success && result.status >= 200 && result.status < 300
      ? confirmedNote(result.data, { actorId, title: request.body.title, body: request.body.body, manualContactIds: request.body.manualContactIds, mentions: request.body.mentions, contactIds: [...request.body.manualContactIds, ...request.body.mentions.map((item) => item.contactId)], eventIds: request.body.eventIds, noteId }) : null;
    if (note) {
      await noteDraftStorage.clear(draftScope);
  ...
  return <AppScreen title="编辑笔记" refreshControl={<RefreshControl refreshing={state.refreshing} onRefresh={state.refresh} />} headerActions={<View style={styles.headerActions}><Pressable accessibilityRole="button" accessibilityLabel="取消编辑笔记" disabled={pending} onPress={() => dirty ? setShowExitPrompt(true) : router.back()} style={styles.cancel}><Text style={styles.cancelText}>取消</Text></Pressable><Pressable accessibilityRole="button" accessibilityLabel={pending ? "保存中" : "保存修改"} accessibilityState={{ disabled: saveDisabled }} disabled={saveDisabled} onPress={() => { void save(); }} style={styles.cancel}><Text style={[styles.headerSaveText, saveDisabled && styles.headerSaveDisabled]}>{pending ? "保存中" : "保存"}</Text></Pressable></View>}>
    {showExitPrompt ? <View accessibilityRole="alert" style={styles.exitPrompt}>
      <Text style={styles.exitTitle}>保留这份修改吗？</Text><Text style={styles.notice}>可以保留后退出、放弃草稿，或继续编辑。</Text>
    ...
    {current ? <>
      ...
      <View style={styles.paper}>
        <TextInput accessibilityLabel="笔记标题" editable={!pending} maxLength={200} value={title} onChangeText={(value) => { setTitle(value); markChanged(); }} style={styles.titleInput} />
      ...
      {draftStatus ? <Text accessibilityLiveRegion="polite" style={draftStatus.startsWith("自动保存失败") ? styles.error : styles.notice}>{draftStatus}</Text> : null}
