...
export interface LiveQrScanConnectGraph {
  contacts: readonly ContactDTO[];
  evidence: readonly RelationshipEvidenceDTO[];
  generatedAt: string;
}

export type LiveQrScanConnectProviderResult<TResult> =
  TResult | Promise<TResult>;

export interface LiveQrScanConnectProvider {
  source: string;
  sourceLabel: string;
  readQrScanConnectGraph: () => LiveQrScanConnectProviderResult<LiveQrScanConnectGraph>;
}

export const QR_SCAN_CONNECT_LIVE_RECORD_COLLECTIONS = {
  contacts: "contacts",
  evidence: "evidence",
} as const;
...
export function createStorageQrScanConnectProvider({
  ...
  store,
  workspaceId,
}: StorageQrScanConnectProviderOptions): LiveQrScanConnectProvider {
  return {
    source: source ?? `live-record-store:qr-scan-connect:${workspaceId}`,
    sourceLabel,
    async readQrScanConnectGraph(): Promise<LiveQrScanConnectGraph> {
