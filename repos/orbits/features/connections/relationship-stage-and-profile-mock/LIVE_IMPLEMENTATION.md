...
- `features/connections/relationship-stage-and-profile-mock/live-service.ts` implements `RelationshipStageAndProfileService` without changing route imports or response shapes.
- `features/connections/relationship-stage-and-profile-mock/providers/stage-automation-provider.ts` owns live stage automation.
- `features/connections/relationship-stage-and-profile-mock/providers/relationship-profiling-provider.ts` owns relationship profiling.
- `features/connections/relationship-stage-and-profile-mock/providers/profile-provenance-store.ts` persists source references, evidence ids, provider run ids, and user-visible explanations.

## Switch Mechanism

- `ORBIT_RELATIONSHIP_PROFILE_PROVIDER=mock` keeps route handlers on `createMockRelationshipStageAndProfileService`.
- `ORBIT_RELATIONSHIP_PROFILE_PROVIDER=live` may switch to `live-service.ts` only after the provider files and replacement tests below exist.
...
- `ORBIT_RELATIONSHIP_PROFILE_PROVIDER`
- `ORBIT_RELATIONSHIP_PROFILE_STORE_URL`
- `ORBIT_RELATIONSHIP_PROFILE_STORE_SERVICE_ROLE`
- `ORBIT_RELATIONSHIP_PROFILE_MODEL`
...
- Live stage automation and relationship profiling must record which source references influenced each field.
- The profile summary must expose whether it came from fixture, deterministic rules, or a live provider.
...
- Profile update success returns `{ success: true, data }` and persists provenance for every field that live automation changes.
