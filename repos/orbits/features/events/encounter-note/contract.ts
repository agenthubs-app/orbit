...
export const EVENT_ENCOUNTER_NOTE_ERROR_CODES = [
  ...
  "EVENT_ENCOUNTER_NOTE_LIVE_STORE_UNCONFIGURED",
] as const;
...
export const EVENT_ENCOUNTER_NOTE_ERROR_DEFINITIONS = {
  ...
  EVENT_ENCOUNTER_NOTE_LIVE_STORE_UNCONFIGURED: {
    code: "EVENT_ENCOUNTER_NOTE_LIVE_STORE_UNCONFIGURED",
    appCode: "SERVICE_UNAVAILABLE",
    message: "The live encounter note store is not configured.",
    recovery:
      "Configure ORBIT_EVENT_DATABASE_URL, ORBIT_LIVE_DATABASE_URL, or ORBIT_DATABASE_URL before using live encounter notes.",
  },
...
export interface EventEncounterNoteProvenance {
  ...
  generationMethod:
    ...
    | "rule-based-failure"
    | "live-store-query"
    | "live-store-note-capture"
    | "live-store-evidence";
  speechToTextRequested: false;
  audioUploadRequested: false;
  liveNoteStorageExecuted: boolean;
  liveDatabaseWriteExecuted: boolean;
...
export type EventEncounterNoteServiceResult<TResult> = TResult | Promise<TResult>;

export interface EventEncounterNoteService {
  createEncounterNote: (
    input?: EventEncounterNoteInput,
  ) => EventEncounterNoteServiceResult<EventEncounterNoteResult>;
  createEncounterEvidence: (
    input?: EventEncounterEvidenceInput,
  ) => EventEncounterNoteServiceResult<EventEncounterEvidenceResult>;
}
