...
export const EVENT_GOAL_READINESS_ERROR_DEFINITIONS = {
  ...
  EVENT_GOAL_READINESS_LIVE_STORE_UNCONFIGURED: {
    code: "EVENT_GOAL_READINESS_LIVE_STORE_UNCONFIGURED",
    appCode: "SERVICE_UNAVAILABLE",
    message: "The live event goal readiness store is not configured.",
    recovery:
      "Configure ORBIT_EVENT_DATABASE_URL, ORBIT_LIVE_DATABASE_URL, or ORBIT_DATABASE_URL before using live event goal readiness.",
  },
...
export type EventGoalReadinessServiceResult<TResult> = TResult | Promise<TResult>;

export interface EventGoalAndReadinessService {
  suggestGoals: (
    input?: EventGoalSuggestionInput,
  ) => EventGoalReadinessServiceResult<EventGoalSuggestionsResult>;
  setGoal: (
    input?: EventGoalSetInput,
  ) => EventGoalReadinessServiceResult<EventGoalSetResult>;
  getReadiness: (
    input?: EventGoalReadinessInput,
  ) => EventGoalReadinessServiceResult<EventGoalReadinessResult>;
}
