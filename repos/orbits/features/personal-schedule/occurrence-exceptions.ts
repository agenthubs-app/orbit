...
export async function readPersonalScheduleOccurrenceExceptions(input: { store: LiveRecordStoreLike; workspaceId: string; actorId: string; seriesId: string; occurrenceDate?: string }): Promise<PersonalScheduleOccurrenceException[]> {
  ...
  const records = await input.store.listRecords({
    workspaceId: input.workspaceId, collectionName: PERSONAL_SCHEDULE_EXCEPTION_COLLECTION, userId: input.actorId, sourceId: input.seriesId,
    ...(input.occurrenceDate === undefined ? { limit: "unbounded" as const } : { limit: 1, recordIds: [`${input.seriesId}:occurrence:${input.occurrenceDate}`] }) });
