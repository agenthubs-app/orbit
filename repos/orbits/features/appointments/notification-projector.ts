...
function revision(value: unknown): number {
  ...
  if (!Number.isSafeInteger(parsed) || parsed < 1) throw new Error("Appointment outbox payload has an invalid revision.");
  return parsed;
}
...
function titleFor(event: AppointmentOutboxEvent, now: string): string | null {
  ...
  switch (eventType) {
    ...
    case "appointment.memo.t15m": return "约谈已经结束：记录会后纪要与下一步";
    default: return null;
  }
}
...
export function createAppointmentNotificationProjector(input: {
  ...
  return {
    async project(event) {
      const actorIds = stringArray(event.payload.participantActorIds);
      if (actorIds.length !== 2) throw new Error("Appointment notification requires exactly two participant actors.");
      ...
      if (!title) return { notificationIds: [], policy: "provider_not_configured" };
...
async function upsertNotification(input: {
  ...
  await input.transaction.query(`
    insert into orbit_records (
      ...
      'task', null, $5, 'active', $6, $7::jsonb, $5, $5, null
    )
    on conflict (workspace_id, collection_name, record_id) do update set
      ...
      deleted_at = null
  `, [input.runtime.workspaceId, input.reminderId, input.actorId, [...input.evidenceIds], input.now, input.title, JSON.stringify(payload)]);
}
...
export function createPostgresAppointmentNotificationProjector(runtime: EventOperationsPostgresRuntime): AppointmentNotificationProjector {
  return {
    async project(event) {
      ...
      const proposalRevision = revision(event.payload.revision);
      if (event.eventType === "appointment.reminders.invalidate") {
        const notificationIds = actorIds.flatMap((actorId) => ["t24h", "t1h", "t15m"].map((type) => `notification:${event.appointmentId}:${proposalRevision}:${type}:${actorId}`));
      ...
      return runtime.client.transaction(async (transaction) => {
        ...
        for (const actorId of actorIds) {
          const reminderId = `notification:${event.appointmentId}:${proposalRevision}:${suffix}:${actorId}`;
          await upsertNotification({ actorId, dueAt: event.availableAt, evidenceIds: [`appointment:${event.appointmentId}:revision:${proposalRevision}`], now: event.createdAt, reminderId, runtime, title, transaction });
          notificationIds.push(reminderId);
        }
