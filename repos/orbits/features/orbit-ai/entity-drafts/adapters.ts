...
export interface ContactDraftCreatePort {
  createManualContactDraft: (input: {
    displayName: string;
    organization?: string | null;
    role?: string | null;
    note?: string | null;
  }) => Promise<
    | { success: true; data: { draft: { draftId: string } | null } }
    | { success: false; error?: { message?: string } }
  >;
}

/**
 * Contacts are the one two-step kind: Orbit has no "create a contact outright"
 * path, so a confirmed draft lands as a contact draft and the card sends the
 * user to the existing confirmation page rather than routing around it.
 */
export function createContactDraftAdapter(port: ContactDraftCreatePort): EntityDraftWriteAdapter {
  return {
    kind: "contact",
    async write({ draft }) {
      const result = await port.createManualContactDraft({
        displayName: required(draft, "name"),
        note: optional(draft, "note") ?? null,
        organization: optional(draft, "organization") ?? null,
        role: optional(draft, "role") ?? null,
      });
      if (result.success !== true) {
        throw new Error(result.error?.message ?? "联系人草稿创建未成功");
      }
      const draftId = result.data.draft?.draftId;
      if (!draftId) throw new Error("联系人草稿没有返回可回读的 id");
