...
export const FOLLOWUP_LIVE_RECORD_COLLECTIONS = {
...
export interface StorageFollowupTaskProviderOptions {
  source?: string;
  sourceLabel?: string;
...
interface CachedConfiguredStorageFollowupTaskProvider {
  key: string;
  provider: LiveFollowupTaskProvider;
}

let cachedDefaultProvider: CachedConfiguredStorageFollowupTaskProvider | null = null;
...
function sourceReference(
  value: unknown,
): ConnectionDTO["source"] | ContactDTO["source"] | TaskDTO["source"] | null {
...
export function createStorageFollowupTaskProvider({
  source,
...
export function createConfiguredStorageFollowupTaskProvider({
  env,
  sourceLabel = "Followup Postgres live storage",
}: ConfiguredStorageFollowupTaskProviderOptions = {}): LiveFollowupTaskProvider | null {
  ...
  const canUseDefaultCache =
    env === undefined && sourceLabel === "Followup Postgres live storage";
  ...
  const provider = createStorageFollowupTaskProvider({
    source: `postgres-live-record-store:followups:${config.workspaceId}`,
    sourceLabel,
