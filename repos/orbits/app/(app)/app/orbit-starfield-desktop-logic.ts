...
// Transplanted verbatim from docs/designs/iOrbit 星空旅程.html
...
export function runStarfieldDesktop(host: HTMLElement): () => void {
  const self: any = { props: {} };
    ...
    const avaURL=(i)=>{const idx=((i%AVA.length)+AVA.length)%AVA.length;return '/iorbit-starfield/avatars/desktop/ava' + idx + '.jpg';};
    // i18n dictionary — zh copy is the desktop reference verbatim (post brand
    // alignment); en mirrors the mobile reference's EN, with desktop-only
    // strings (kicker, ending paragraphs, scroll cue) translated to match.
    const DICT={
      zh:{
        ...
        leftParaHtml:'<span style="display:block;white-space:nowrap;">那些躺在名片夹里的人，</span><span style="display:block;white-space:nowrap;">其实是一片待点亮的星空。</span><span style="display:block;white-space:nowrap;">iOrbit 替你逐颗解读，</span><span style="display:block;white-space:nowrap;">挖出每段关系背后的<b style="font-weight:500;color:#fff;">商业价值</b>。</span>',
        rightParaHtml:'<span style="display:block;white-space:nowrap;">让一场活动，</span><span style="display:block;white-space:nowrap;">从「人多」变成<b style="font-weight:500;color:#fff;">「人对」</b>。</span><span style="display:block;white-space:nowrap;">iOrbit 替每位来宾算好轨道，</span><span style="display:block;white-space:nowrap;">落座就在对的人中间。</span>'
      },
      en:{
        ...
        slo2:'Hundreds of cards at one event — only a few matter. Orbit finds them for you.',
        ...
        cueNext:'Scroll / Space · next scene', cueLast:'Last scene · scroll to restart',
        ...
        leftParaHtml:'<span style="display:block;white-space:nowrap;">The people in your card holder</span><span style="display:block;white-space:nowrap;">are a starfield waiting to be lit.</span><span style="display:block;white-space:nowrap;">iOrbit reads them one by one,</span><span style="display:block;white-space:nowrap;">surfacing the <b style="font-weight:500;color:#fff;">business value</b> in each.</span>',
    ...
    // NOTE: inline onerror string — `this` here is the <img> element in the page, not the component.
    const AVAFALL="this.removeAttribute('src');this.style.background='radial-gradient(circle at 34% 30%,#cfc6ff,#8b7bf0 72%)';";
    ...
    const demoData=()=>{const a=[card(2),card(11),card(20)];a[2]._gold=true;return a;};
    demoWrap.innerHTML=''; const demoEls=demoData().map((d)=>{const e=document.createElement('div');e.style.cssText='opacity:0;transform:translateY(16px) scale(.96);will-change:transform,opacity;pointer-events:auto;';e.innerHTML=cardHTML(d,!!d._gold,256);demoWrap.appendChild(e);return e;});
    ...
    const buildA=()=>{
      ...
      const mk=(sx,sy,vlo,vhi,sz)=>{const aa=rnd()*6.283,rr=Math.pow(rnd(),0.62);return{nx:Math.cos(aa)*rr*sx,ny:Math.sin(aa)*rr*sy,v:vlo+Math.pow(rnd(),2.2)*(vhi-vlo),sz,depth:0.3+rnd()*0.7,twf:6e-4+rnd()*13e-4,twp:rnd()*6.28,fr:1e-4+rnd()*3e-4,ph:rnd()*6.28,px:0,py:0};};
      ...
      for(let i=0;i<32;i++){const aa=(i/32)*6.283+rnd()*0.32,rr=0.26+rnd()*0.58;const b=mk(0,0,0.6,1,3.0);b.nx=Math.cos(aa)*rr*0.98;b.ny=Math.sin(aa)*rr*0.76;b.v=0.5+rnd()*0.5;b.data=card(i*4+5);b.bridge=(i%7===0);b.cl=i%3;b.jnx=(rnd()*2-1)*(0.6+rnd()*0.85);b.jny=(rnd()*2-1)*(0.6+rnd()*0.85);A.bright.push(b);}
    ...
    const buildC=()=>{
      ...
      for(let i=0;i<560;i++){const aa=rnd()*6.283,rr=Math.pow(rnd(),0.5)*0.98;C.stars.push({nx:Math.cos(aa)*rr,ny:Math.sin(aa)*rr*0.72,v:Math.pow(rnd(),2.0)*0.46,depth:0.3+rnd()*0.7,twf:6e-4+rnd()*13e-4,twp:rnd()*6.28,px:0,py:0,data:card(i*3+7)});}
      const tg=T().relTags;
      for(let i=0;i<6;i++){const d=card(i*9+3);d.tag=tg[i];if(i===0)d._gold=true;C.rel.push({v:0.6+rnd()*0.4,gold:(i===0),data:d,hnx:0,hny:0,px:0,py:0,dep:0.5,phi:rnd()*6.28,phi2:rnd()*6.28});}
    ...
    // orbit cards
    let relCards=[];
    const buildRelCards=()=>{orbitWrap.innerHTML='';relCards=C.rel.map((s)=>{const e=document.createElement('div');e.style.cssText='position:absolute;left:0;top:0;opacity:0;will-change:transform,opacity;pointer-events:auto;';e.innerHTML=cardHTML(s.data,s.gold,248);orbitWrap.appendChild(e);return e;});};
    ...
    const resize=()=>{
      ...
      cx=W/2;cyc=H*0.41;minDim=Math.min(W,H);spread=minDim*0.95;Rneb=minDim*0.27;RX=Rneb*1.52;RY=Rneb*0.82;orx=RX*0.80;ory=RY*0.86;
      const fr=fieldWrap.getBoundingClientRect();
      cardY=isFinite(fr.top)&&fr.top>0?clamp(fr.top-Math.min(H*0.13,130),H*0.40,H*0.60):H*0.52;
      clusterY=cardY-Math.min(H*0.2,170);demoWrap.style.top=cardY+'px';
    };
    ...
    const renderA=(p,t)=>{
      ...
      const drawAmb=(arr,gk)=>{for(let i=0;i<arr.length;i++){const s=arr[i];const x=cx+s.nx*spread+Math.sin(t*s.fr+s.ph)*minDim*0.008,y=cyc+s.ny*spread+Math.cos(t*s.fr*0.9+s.ph)*minDim*0.008;s.px=x;s.py=y;const tw=0.6+0.4*Math.sin(t*s.twf+s.twp);drawDot(x,y,s.sz*(0.7+0.3*s.depth),starHSL(s.v),(0.25+0.65*s.v)*tw*(1-dim*0.7),gk);}};
    ...
    const renderC=(p,t)=>{
      ...
      for(let i=0;i<6;i++){const s=C.rel[i];
        ...
        const isHover=(hoverStar===s);drawDot(px,py,(3.2+(isHover?1.4:0))*(0.72+0.45*dep),s.gold?'hsl(42,56%,76%)':starHSL(0.5),reveal*lerp(0.5,1,arc)*(0.6+0.4*dep),5.4);}
    ...
    const updateOverlays=(p,t,elapsed,dt,idle)=>{
      ...
      if(p<0.30){const a=clampr(elapsed,3.5,0.7)*(1-clampr(elapsed,7.0,0.9));prOp=a*ch1;prTxt=PROC1;const fr=fieldWrap.getBoundingClientRect();prTop=(isFinite(fr.bottom)&&fr.bottom>0?fr.bottom:H*0.78)+76;}
      else{const a=smooth(p,0.51,0.58)*(1-smooth(p,0.74,0.80));prOp=a;prTxt=PROC2;prTop=cyc+ory+30;}
      ...
      if(stopIdx===0 && !animating && p<0.05 && !pinned && !hoverStar && A.bright.length){
        carT+=dt;
      ...
      // demo cards
      demoEls.forEach((el,i)=>{const a=smooth(p,0.15+i*0.022,0.20+i*0.022)*(1-smooth(p,0.24,0.30));el.style.opacity=a.toFixed(3);el.style.transform='translateY('+((1-a)*16)+'px) scale('+(0.96+0.04*a)+')';});
      // fog DOM blur
      ...
      relCards.forEach((el,i)=>{const s=C.rel[i];if(!s){el.style.opacity='0';return;}
        ...
        const sc=lerp(0.55,1.02,dep);
        const cw=248*sc, chh=(el.offsetHeight||186)*sc;
        const ux=s.px-cx,uy=s.py-cyc,ul=Math.hypot(ux,uy)||1;
        const ox=s.px+ux/ul*minDim*0.21, oyy=s.py+uy/ul*minDim*0.16;
        const x=clamp(ox-cw/2,12,Math.max(12,W-cw-12)), y=clamp(oyy-chh/2,136,Math.max(136,H-186-chh));
        const a=appear;
        el.style.opacity=a.toFixed(3);el.style.zIndex=String(3+Math.round(dep*9));
        el.style.transformOrigin='top left';el.style.filter='none';
        el.style.transform='translate('+x.toFixed(0)+'px,'+y.toFixed(0)+'px) scale('+sc.toFixed(3)+')';
    ...
    const frame=(p,t,dt,idle)=>{
      ...
      let wb=ctx.createRadialGradient(W*0.5,H*0.32,0,W*0.5,H*0.32,minDim*1.05);wb.addColorStop(0,'rgba(90,80,200,0.08)');wb.addColorStop(1,'rgba(90,80,200,0)');ctx.fillStyle=wb;ctx.fillRect(0,0,W,H);
      if(p<0.33)renderA(p,t);else renderC(p,t);
      let vg=ctx.createRadialGradient(cx,cyc,minDim*0.32,cx,cyc,minDim*0.95);vg.addColorStop(0,'rgba(5,4,11,0)');vg.addColorStop(1,'rgba(4,3,9,0.62)');ctx.fillStyle=vg;ctx.fillRect(0,0,W,H);
      renderFog(p,t);
      if(showGrain){if(!self._np)self._np=ctx.createPattern(noise,'repeat');ctx.save();ctx.globalAlpha=0.04;ctx.globalCompositeOperation='overlay';const ox=(t*0.005)%170;ctx.translate(ox,ox);ctx.fillStyle=self._np;ctx.fillRect(-ox-170,-ox-170,W+340,H+340);ctx.restore();}
    ...
    const applyLang=(lng,persist)=>{
      try{
        ...
        rebuildDemo();rebuildSteps();buildRelCards();if(self._setMini)self._setMini();
  ...
  return () => {if(self._raf)cancelAnimationFrame(self._raf);if(self._miniIv)clearInterval(self._miniIv);if(self._cleanup)self._cleanup();if(self._promptCleanup)self._promptCleanup();if(self._langStyle)self._langStyle.remove();};
}
