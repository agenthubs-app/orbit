...
type AppointmentView = {
  ...
  confirmed: null | {
    ...
    medium: { joinUrl?: string | null; kind: string; provider?: string };
...
function ProposalForm({ busy, counter, onSubmit }: { busy: boolean; counter: boolean; onSubmit: (value: { candidateTimes: { startsAtUtc: string }[]; durationMinutes: number; medium: { kind: "video"; provider: "google_meet"; joinUrl: null }; note: string; timezone: string }) => Promise<void> }) {
  ...
  const updateSlot = (index: number, value: string) => {
    setSlots((values) => values.map((current, candidate) => candidate === index ? value : current));
  };
  return (
    <form onSubmit={(event) => { event.preventDefault(); if (parsed.every((value): value is string => Boolean(value))) void onSubmit({ candidateTimes: parsed.map((startsAtUtc) => ({ startsAtUtc })), durationMinutes: duration, medium: { kind: "video", provider: "google_meet", joinUrl: null }, note, timezone }); }} style={{ display: "grid", gap: 9 }}>
      ...
      {slots.map((slot, index) => <input aria-label={t({ en: `Candidate ${index + 1}`, zh: `候选时间 ${index + 1}` })} className="field" key={index} min={localDateTimeValue(new Date(Date.now() + 30 * 60_000))} onInput={(event) => updateSlot(index, event.currentTarget.value)} type="datetime-local" value={slot} />)}
...
export function OrbitAppointmentNegotiation({
  ...
  return (
    <section className="card-flat" data-appointment-deep-link={appointmentId || undefined} data-appointment-negotiation style={{ display: "grid", gap: 11, padding: 14 }}>
      ...
      {appointment ? <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}><span className="chip">{appointment.status}</span><span className="chip">v{appointment.version}</span>{appointment.projection.calendar === "synced" ? <span className="chip">{t({ en: "Calendar synced", zh: "日历已同步" })}</span> : appointment.projection.calendar === "failed" ? <span className="chip">{t({ en: "Calendar sync failed", zh: "日历同步失败" })}</span> : appointment.projection.calendar === "not_synced" ? <span className="chip">{t({ en: "Calendar not synced", zh: "日历未同步" })}</span> : null}{appointment.projection.meeting === "synced" ? <span className="chip">{t({ en: "Meet ready", zh: "会议已创建" })}</span> : appointment.projection.meeting === "failed" ? <span className="chip">{t({ en: "Meet creation failed", zh: "会议创建失败" })}</span> : appointment.projection.meeting === "not_synced" ? <span className="chip">{t({ en: "Meet not synced", zh: "会议未同步" })}</span> : null}</div> : null}
      ...
      {appointment?.confirmed ? <div className="card-flat" style={{ display: "grid", gap: 7, padding: 11 }}><strong>{new Intl.DateTimeFormat(undefined, { dateStyle: "full", timeStyle: "short", timeZone: appointment.confirmed.timezone }).format(new Date(appointment.confirmed.startsAtUtc))}</strong><span style={{ color: "var(--text-2)", fontSize: 13 }}>{appointment.confirmed.durationMinutes} min · {appointment.confirmed.timezone}</span>{appointment.confirmed.medium.joinUrl ? <a className="btn btn-primary btn-sm" href={appointment.confirmed.medium.joinUrl} rel="noreferrer" style={{ justifySelf: "start" }} target="_blank">{t({ en: "Join Google Meet", zh: "加入 Google Meet" })}</a> : null}<span style={{ color: "var(--text-3)", fontSize: 12 }}>{t({ en: "In-app reminders: T−24h, T−1h; memo prompt: T+15m", zh: "站内提醒：提前 24 小时、提前 1 小时；会后 15 分钟提醒记录" })}</span></div> : null}
