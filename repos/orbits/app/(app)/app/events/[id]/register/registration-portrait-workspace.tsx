...
export function RegistrationPortraitWorkspace({ actorId, event, language, children, enrollment, initialDraftAnswers }: RegistrationPortraitWorkspaceProps) {
  ...
  return <section data-portrait-view={view} className="registration-portrait-7a" style={viewportHeight ? { height: viewportHeight } : undefined}>
    <style>{`.registration-portrait-7a{background:#fff;color:#0B1220;min-height:100dvh;font-family:var(--ff);padding:0 16px 16px}.registration-portrait-7a>header,.registration-portrait-7a>article,.registration-portrait-7a>footer{max-width:760px;margin:auto}.registration-portrait-7a>header{min-height:48px;display:flex;align-items:center;justify-content:space-between;gap:8px}.registration-portrait-7a button{font:inherit;cursor:pointer;min-height:44px}.registration-portrait-7a button:disabled{cursor:default;opacity:.45}.registration-portrait-7a .portrait-link{background:transparent;border:0;color:#0A5CFF;padding:10px 8px;font-size:13px}.registration-portrait-7a .portrait-primary{background:#0B1220;color:#fff;border:1px solid #0B1220;border-radius:12px;min-height:50px;padding:12px 18px;font-weight:800}.registration-portrait-7a .portrait-entry{display:flex;align-items:center;justify-content:space-between;width:100%;background:#F5F7FA;border:0;border-radius:10px;text-align:left;padding:14px;margin:16px 0;gap:16px}.registration-portrait-7a>article{padding:16px 0}.registration-portrait-7a dl>div{display:flex;gap:8px;padding:9px 0;border-bottom:1px solid #EEF0F4}.registration-portrait-7a dt{width:100px;flex-shrink:0;font-size:12px;color:#8B93A5}.registration-portrait-7a dd{margin:0;font-size:14px;line-height:20px;font-weight:600}.registration-portrait-7a>footer{border-top:1px solid #E6E8EE;padding-top:12px;display:flex;flex-wrap:wrap;gap:8px}.registration-portrait-7a .portrait-privacy{font-size:11px;color:#0A5CFF;background:#EEF3FF;padding:4px 8px;display:inline-block;border-radius:6px}.registration-portrait-7a h2{font-size:24px;line-height:32px;font-weight:900}.registration-portrait-7a .portrait-note{font-size:12px;line-height:19px;color:#6B7280}.registration-portrait-7a [role=alert]{color:#B42318;font-size:13px}.registration-portrait-7a .portrait-progress{height:2px;background:#EEF0F4}.registration-portrait-7a .portrait-progress>span{display:block;height:2px;background:#0B1220}.registration-portrait-7a [data-orbit-registration-profile-guide]{background:#fff!important;padding:16px 0!important;min-height:0!important}.registration-portrait-7a [data-reg-saved-registration],.registration-portrait-7a [data-reg-cancelled-registration],.registration-portrait-7a [data-reg-anim=question]{border:0!important;border-radius:0!important;box-shadow:none!important;background:#fff!important}`}</style>
    ...
    {view === "registration" ? <article>
      {enrollment ? <main data-registration-stage={enrollment.stage} data-registration-status={enrollment.status}>
        ...
        {source?.questionSet.questions.map((formal, index) => <section key={formal.id} style={{ padding: "14px 0", borderBottom: "1px solid #EEF0F4" }}>
      ...
      {view === "result" ? <>
        ...
        {view === "interview" ? <section>
          <p className="portrait-note" style={{ color: "#0A5CFF" }}>IORBIT</p>
...
function PortraitQuestionInput({ prompt, options, answer, onChange, language = "en", formal = false }: { prompt: string; options: readonly string[]; answer: string; onChange: (value: string) => void; language?: "en" | "zh"; formal?: boolean }) {
  ...
  return <div>
    ...
    <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>{choices.map(option => <button className="btn" type="button" key={option} aria-pressed={!other && answer === option} onClick={() => { setOther(false); onChange(option); }} style={{ border: "1px solid #E6E8EE", borderRadius: 8, padding: "8px 12px", fontSize: formal ? 13 : 14, color: !other && answer === option ? "#fff" : "#475569", background: !other && answer === option ? "#0B1220" : "#fff" }}>{option}</button>)}{choices.length ? <button className="btn" type="button" aria-pressed={other} onClick={() => { setOther(true); onChange(custom); }} style={{ border: "1px solid #E6E8EE", borderRadius: 8, padding: "8px 12px", fontSize: formal ? 13 : 14, background: other ? "#0B1220" : "#fff", color: other ? "#fff" : "#475569" }}>{language === "en" ? "Other" : "其他"}</button> : null}</div>
    {other ? <textarea aria-label={language === "en" ? "Your own answer" : "自行补充回答"} rows={2} value={custom} maxLength={1000} onChange={event => { setCustom(event.target.value); onChange(event.target.value); }} style={{ boxSizing: "border-box", width: "100%", minHeight: 44, border: 0, borderBottom: "1.5px solid #0B1220", background: "#fff", padding: "8px 0", font: "inherit", resize: "vertical" }} /> : null}
  </div>;
}
