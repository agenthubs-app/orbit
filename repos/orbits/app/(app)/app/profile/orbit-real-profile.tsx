...
const textFields = [
  { key: "fullName", label: "姓名", icon: "user" },
  { key: "headline", label: "一句话介绍", icon: "sparkle" },
  { key: "company", label: "公司", icon: "wallet" },
  { key: "title", label: "职位", icon: "star" },
  { key: "wechatName", label: "微信号", icon: "mail" },
  { key: "lineId", label: "LINE ID", icon: "mail" },
] as const;
...
function ProfileMethods({
  ...
  const methods = [
    ["ai", "sparkle", "AI 文本提取", "粘贴简介自动填"],
    ["scan", "search", "名片扫描", "拍一张自动识别"],
    ["manual", "user", "手动填写", "一个个字段填"],
  ] as const;
    ...
    <div>
      ...
      {method === "ai" ? (
        <div style={{ marginTop: 14 }}>
          <textarea className="field" onChange={(event) => setExtractText(event.target.value)} placeholder="粘贴业务、经历、关注方向或希望认识的人" style={{ fontFamily: "var(--ff)", height: 88, lineHeight: 1.5, padding: 12, resize: "none" }} value={extractText} />
          <div style={{ color: "var(--text-3)", fontSize: 12.5, marginTop: 8 }}>会结合下方已填写的公司和职位提取，结果只填入表单，确认后再保存。</div>
          <button className="btn btn-dark" disabled={extracting} onClick={onTextExtract} style={{ marginTop: 10 }} type="button"><Icon name="sparkle" size={16} />{extracting ? "提取中…" : "提取到表单"}</button>
        </div>
      ) : null}
      {method === "scan" ? (
        <button disabled={extracting} onClick={onFilePick} style={{ background: "var(--surface-2)", border: "1.5px dashed var(--border-strong)", borderRadius: 14, cursor: "pointer", fontFamily: "var(--ff)", marginTop: 14, padding: 24, textAlign: "center", width: "100%" }} type="button">
          ...
          <div style={{ color: "var(--ink)", fontSize: 14, fontWeight: 550 }}>{extracting ? "正在提取…" : "点击上传名片或简历"}</div>
          <div style={{ color: "var(--text-3)", fontSize: 12.5, marginTop: 4 }}>支持 JPG / PNG / PDF，结果只填入表单，确认后再保存。</div>
        </button>
      ) : null}
    </div>
  );
...
function ChipGroup({
  ...
  return (
    <div>
      <label className="field-label">{label}（{values.length}）</label>
...
function ProfileFields({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, marginTop: 22 }}>
      ...
      <div><label className="field-label">一句话简介</label><textarea className="field" onChange={(event) => update("bio", event.target.value)} style={{ fontFamily: "var(--ff)", height: 64, lineHeight: 1.5, padding: 12, resize: "none" }} value={profile.bio} /></div>
      <ChipGroup label="我能提供 (offering)" onToggle={toggleTag} options={viewModel.offeringTags} section="offering" values={profile.offering} />
      <ChipGroup label="我想寻求 (seeking)" onToggle={toggleTag} options={viewModel.seekingTags} section="seeking" values={profile.seeking} />
      <ChipGroup label="想聊的话题 (topics)" onToggle={toggleTag} options={viewModel.topics} section="topics" values={profile.topics} />
      <div><label className="field-label">开场白 (intro)</label><textarea className="field" onChange={(event) => update("intro", event.target.value)} style={{ fontFamily: "var(--ff)", height: 56, lineHeight: 1.5, padding: 12, resize: "none" }} value={profile.intro} /></div>
    </div>
  );
}
...
export function OrbitRealProfile({ viewModel }: { viewModel: OrbitProfileViewModel }) {
  ...
  const missing = useMemo(() => {
    ...
    if (!profile.fullName.trim()) fields.push("姓名");
    if (!profile.wechatName.trim() && !profile.lineId.trim()) fields.push("微信或 LINE");
    if (!profile.company.trim()) fields.push("公司");
    if (!profile.title.trim()) fields.push("职位");
    if (!profile.industry.trim()) fields.push("行业");
    if (!profile.bio.trim()) fields.push("简介");
    if (!profile.intro.trim()) fields.push("开场白");
    if (!profile.offering.length) fields.push("能提供");
    if (!profile.seeking.length) fields.push("想寻求");
    if (!profile.topics.length) fields.push("话题");

    return fields;
  }, [profile]);
  const alert = message ? (
    <div style={{ background: "var(--live-soft)", borderRadius: 10, color: "var(--live)", fontSize: 13, padding: "10px 12px" }}>{message}</div>
  ) : missing.length ? (
    <div style={{ background: "var(--amber-soft)", borderRadius: 10, color: "var(--amber)", fontSize: 13, padding: "10px 12px" }}>还差：{missing.join("、")}（填全才能被匹配）</div>
  ) : (
    <div style={{ background: "var(--live-soft)", borderRadius: 10, color: "var(--live)", fontSize: 13, padding: "10px 12px" }}>档案完整，可被匹配</div>
  );
  ...
  const methodsProps = { extractText, extracting, method, onFilePick: fakeExtract, onTextExtract: fakeExtract, setExtractText, setMethod };
    ...
    <main data-orbit-real-page="profile">
      <div className="orbit-desktop-only scroll" data-appscroll style={{ background: "var(--bg)", minHeight: "100dvh", overflowY: "auto", position: "relative" }}>
        ...
        <form onSubmit={onSubmit}>
          <div style={{ margin: "0 auto", maxWidth: 720, padding: "36px 40px 110px" }}>
            <button className="btn btn-quiet btn-sm" onClick={() => orbitNavigate("/home")} style={{ marginBottom: 16, paddingLeft: 8 }} type="button"><Icon name="chevL" size={16} />返回</button>
            <div style={{ alignItems: "center", display: "flex", gap: 16, marginBottom: 6 }}>
              <Avatar letter={letter} size={56} />
              <div style={{ minWidth: 0 }}><h1 className="h-display" style={{ fontSize: 30, margin: 0 }}>通用档案</h1><div style={{ color: "var(--text-2)", fontSize: 14, marginTop: 3 }}>{subText}</div></div>
            </div>
            ...
            <ProfileFields profile={profile} selectRenderKey={selectRenderKey} toggleTag={toggleTag} update={update} viewModel={viewModel} />
          </div>
          <div style={{ backdropFilter: "blur(14px)", background: "rgba(255,255,255,0.92)", borderTop: "1px solid var(--border)", bottom: 0, display: "flex", gap: 12, justifyContent: "flex-end", left: 0, padding: "14px 40px", position: "absolute", right: 0 }}>
            <button className="btn btn-ghost" onClick={() => orbitNavigate("/home")} type="button">取消</button>
            <button className="btn btn-primary" disabled={saving} type="submit"><Icon color="#fff" name="check" size={16} />{saving ? "保存中…" : "保存档案"}</button>
          </div>
        </form>
      </div>
      <div className="orbit-mobile-only" style={{ background: "var(--bg)", display: "flex", flexDirection: "column", minHeight: "100dvh", position: "relative" }}>
        ...
        <form onSubmit={onSubmit} style={{ display: "flex", flex: 1, flexDirection: "column", minHeight: 0 }}>
          <MobileBar onBack={() => orbitNavigate("/home")} right={<button className="btn btn-primary btn-sm" disabled={saving} type="submit">{saving ? "保存中…" : "保存"}</button>} title="通用档案" />
