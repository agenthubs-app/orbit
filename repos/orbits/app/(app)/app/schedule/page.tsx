 ...
 * `schedule-route-view-model.ts`，避免 presenter 直接读取 feature DTO。
 */
import {
...
export default async function AppSchedulePage({
  ...
  return (
    <>
      ...
      {localizedRouteModel.state === "success" ? (
        <AppScheduleSuccessView
          arrangements={localizedRouteModel.arrangements}
          evidenceIds={localizedRouteModel.evidenceIds}
          summary={localizedRouteModel.summary}
        />
      ) : (
        <ScheduleRouteStateView routeState={localizedRouteModel.routeState} />
      )}
