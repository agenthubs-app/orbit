...
function HostPortfolioCard({ event, language, t }: { event: OrbitAdminEventView; language: OrbitLanguage; t: OrbitT }) {
  return <div className="card orbit-host-portfolio-card"><div className="orbit-host-portfolio-card-main"><EventCover className="orbit-host-portfolio-cover" g={event.g} imageAlt={event.name} imageSizes="64px" imageUrl={eventCoverPhoto(event.code)} monogram={eventCoverPhoto(event.code) ? null : { text: event.name.slice(0, 1), size: 24 }} /><div className="orbit-host-portfolio-info"><div className="orbit-host-title-row"><strong>{event.name}</strong><StatusBadge language={language} status={event.status} /></div><div className="orbit-host-portfolio-count"><span>{formatOrbitDateTime(event.startsAt, language)}</span><span>{event.venue}</span></div><p className="orbit-host-muted">{event.summary || t({ en: "No source summary", zh: "暂无来源摘要" })}</p></div></div></div>;
}
