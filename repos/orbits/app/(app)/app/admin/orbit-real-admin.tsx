  ...
  return (
    <div className="orbit-host-modal-layer">
      ...
      <div aria-label={t({ en: "Create event", zh: "新建活动" })} aria-modal="true" className="card orbit-host-create-modal" ref={cardRef} role="dialog" tabIndex={-1}>
        ...
        <div className="orbit-host-create-body"><div className="eyebrow">STEP 0{stepN + 1} / 0{steps.length} · {steps[stepN]}</div>{stepN === 0 ? <><label className="orbit-host-field"><span className="field-label">{t({ en: "Event name", zh: "活动名称" })}</span><input className="field" defaultValue={initialEvent?.name ?? ""} /></label><div className="orbit-host-two-col"><label className="orbit-host-field"><span className="field-label">{t({ en: "Event code", zh: "活动编号" })}</span><input className="field" defaultValue={initialEvent?.code ?? ""} /></label><label className="orbit-host-field"><span className="field-label">{t({ en: "Theme color", zh: "主题色" })}</span><input className="field" defaultValue={initialEvent?.themeColor ?? "#6359E9"} /></label></div><label className="orbit-host-field"><span className="field-label">{t({ en: "One-line summary", zh: "一句话简介" })}</span><input className="field" defaultValue={initialEvent?.summary ?? ""} /></label></> : stepN === 1 ? <><div className="orbit-host-two-col"><label className="orbit-host-field"><span className="field-label">{t({ en: "Start time", zh: "开始时间" })}</span><input className="field" defaultValue={initialEvent?.startsAt ?? ""} /></label><label className="orbit-host-field"><span className="field-label">{t({ en: "End time", zh: "结束时间" })}</span><input className="field" defaultValue={initialEvent?.endsAt ?? ""} /></label></div><label className="orbit-host-field"><span className="field-label">{t({ en: "Venue", zh: "场地" })}</span><input className="field" defaultValue={initialEvent?.venue ?? ""} /></label><label className="orbit-host-field"><span className="field-label">{t({ en: "Full address", zh: "详细地址" })}</span><input className="field" defaultValue={initialEvent?.venue ?? ""} /></label></> : stepN === 2 ? <><div className="orbit-host-automation-note"><Icon color="var(--accent)" name="sparkle" size={17} /><span>{t({ en: "Registration questions apply only to this event; the general profile is reused for matching automatically.", zh: "报名问题只用于本场活动；通用画像会自动复用于匹配。" })}</span></div>{formQuestions.map(([question, required]) => <div className="orbit-host-automation-row" key={question}><span style={{ border: "1.5px solid var(--border-2)", borderRadius: 5, height: 18, width: 18 }} /><div><strong>{question}</strong><span>{required}</span></div><span className="chip">{required}</span></div>)}</> : <><div className="orbit-host-automation-note"><Icon color="var(--accent)" name="zap" size={17} /><span>{t({ en: "Automatically opens check-in and runs AI grouping and recommendations on schedule.", zh: "到点自动开启签到、自动运行 AI 分组与推荐。" })}</span></div>{automationRows.map(([action, time]) => <div className="orbit-host-automation-row" key={action}><Icon color="var(--accent)" name="clock" size={16} /><div><strong>{action}</strong><span>{time}</span></div><span className="chip chip-accent">{t({ en: "Auto", zh: "自动" })}</span></div>)}</>}</div>
