...
const regStyles: Record<string, CSSProperties> = {
  ...
  secondary: { background: "var(--surface)", border: "1px solid var(--border-2)", borderRadius: "var(--r-md)", color: "var(--text-2)", cursor: "pointer", fontFamily: "var(--ff)", fontSize: 15, fontWeight: 600, height: 48, marginTop: 10, width: "100%" },
...
function RegMethodCard({
  ...
  return (
    ...
    >
      ...
      <div style={{ color: active ? "var(--accent)" : "var(--ink)", fontSize: 15, fontWeight: 600, marginTop: 4 }}>{title}</div>
    ...
    <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: "var(--r-lg)", boxShadow: "var(--sh-md)", overflow: "hidden" }}>
      <Cover g={gradientFromString(event.code)} imageAlt={event.name} imageUrl={eventCoverPhoto(event.code)} style={{ height: 92, position: "relative" }}>
        <div style={{ display: "flex", flexDirection: "column", inset: 0, justifyContent: "space-between", padding: 16, position: "absolute" }}>
          ...
          <div style={{ color: "var(--on-dark)", fontFamily: "var(--ff-tight)", fontSize: 20, fontWeight: 600 }}>{profile.name || t({ en: "Alex Lee", zh: "李明" })}</div>
        </div>
      </Cover>
      <div style={{ alignItems: "center", display: "flex", gap: 14, padding: 16 }}>
        <div style={{ flex: 1 }}>
          ...
          <div className="mono" style={{ color: "var(--ink)", fontSize: 26, fontWeight: 600, letterSpacing: "0.1em" }}>{code}</div>
          <div style={{ color: "var(--text-3)", fontSize: 12, marginTop: 6 }}>{profile.company} · {profile.title}</div>
        </div>
        <div style={{ alignItems: "center", background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: "var(--r-sm)", color: "var(--ink)", display: "flex", height: 72, justifyContent: "center", width: 72 }}>
      ...
      <div style={regStyles.shell}>
          ...
          <div style={regStyles.screen}>
            ...
            {sourceMode === "describe" ? (
              <div style={{ display: "grid", gap: 12 }}>
                ...
                <label>
                  <div style={{ color: "var(--text-2)", fontSize: 13, fontWeight: 600, marginBottom: 7 }}>{t({ en: "Business and experience", zh: "业务与经历" })}</div>
              ...
              <div style={{ display: "grid", gap: 12 }}>
                <div style={{ background: "var(--surface-2)", border: "1.5px dashed var(--border-strong)", borderRadius: 16, padding: 28, textAlign: "center" }}>
                  ...
                  <div style={{ color: "var(--text-2)", fontSize: 14, lineHeight: 1.5, marginTop: 10 }}>{t({ en: `Upload a ${sourceMode === "resume" ? "PDF resume" : "business card"}, and AI will pre-fill your name, company, title, industry and tags.`, zh: `上传一张${sourceMode === "resume" ? "PDF 简历" : "名片"}，AI 会把姓名、公司、职位、行业和标签先帮你填好。` })}</div>
                </div>
              ...
              {([
                ...
                return (
                  <div key={label} style={{ alignItems: "center", display: "flex", gap: 12 }}>
                    <div style={{ alignItems: "center", background: done ? "var(--accent-soft)" : "transparent", border: `1.5px solid ${done || working ? "var(--accent)" : "var(--border-2)"}`, borderRadius: "var(--r-pill)", color: "var(--accent)", display: "flex", height: 22, justifyContent: "center", width: 22 }}>
                      {done ? <Icon name="check" size={13} /> : null}
                    </div>
                    <div style={{ color: "var(--ink)", flex: 1, fontSize: 14 }}>{label}</div>
                    <div className="mono" style={{ color: "var(--text-4)", fontSize: 12 }}>{working ? "..." : note}</div>
                  </div>
                );
              })}
            </div>
        ...
        {screen === "confirm" ? (
          <div style={regStyles.screen}>
            ...
            <div style={{ display: "grid", gap: 14 }}>
                ...
                <div style={{ background: "var(--surface-2)", border: "1px solid var(--border)", borderRadius: "var(--r-sm)", display: "inline-flex", marginBottom: 9, padding: 3 }}>
                  {([["wechat", { en: "WeChat", zh: "微信号" }], ["line", { en: "LINE ID", zh: "LINE 号" }]] as const).map(([key, labelCopy]) => (
                    <button
                      ...
                      style={{ background: contactMethod === key ? "var(--ink)" : "none", border: "none", borderRadius: 8, color: contactMethod === key ? "var(--on-dark)" : "var(--text-2)", cursor: "pointer", fontFamily: "var(--ff)", fontSize: 13, fontWeight: 600, padding: "6px 14px" }}
                      type="button"
                    >
                      {t(labelCopy)}
                    </button>
              ...
              <label>
                <div style={{ color: "var(--text-2)", fontSize: 13, fontWeight: 600, marginBottom: 7 }}>{t({ en: "Seniority", zh: "职级" })}</div>
              ...
              <label>
                <div style={{ color: "var(--text-2)", fontSize: 13, fontWeight: 600, marginBottom: 7 }}>{t({ en: "Experience and background", zh: "经历与背景" })}</div>
        ...
        {screen === "complete" ? (
          <div style={regStyles.screen}>
            ...
            <div style={{ color: "var(--text-3)", fontSize: 13, lineHeight: 1.5, textAlign: "center" }}>{t({ en: "If you can't find the screenshot later, that's okay — staff can verify you with your email or pass code.", zh: "如果一时找不到截图也没关系，工作人员可以用你的邮箱或通行码帮你核对。" })}</div>
          </div>
        ) : null}
      </div>
    </main>
