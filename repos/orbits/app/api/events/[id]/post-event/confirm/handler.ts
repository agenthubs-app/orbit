...
async function readPostEventReviewConfirmInput(
  ...
  const queryInput: ConfirmPostEventContactsInput = {
    contactDraftIds: splitContactDraftIds(
      url.searchParams.get("contactDraftIds"),
    ),
    eventId,
...
export function createPostEventConfirmPostHandler(
  dependencies: OwnedEventAccessDependencies = {},
) {
  return withOwnedEventAccess(async function confirmPostEventContacts(
    request: Request,
    _context: PostEventReviewConfirmRouteContext,
    access,
  ): Promise<Response> {
    const result =
      await createPostEventContactReviewService().confirmPostEventContacts(
        await readPostEventReviewConfirmInput(request, access.eventId),
      );
