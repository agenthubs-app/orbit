...
export function createTaskSuggestionSnoozePostHandler(
  ...
  return async function POST(
    ...
    try {
      ...
      suggestionExactKeys(body, ["idempotencyKey", "nextVisibleAt"]);
      const suggestion = await suggestionService(dependencies).snooze({
        actorId: actor.id,
        suggestionId: id,
        idempotencyKey: suggestionString(body.idempotencyKey, "idempotencyKey"),
        nextVisibleAt: suggestionString(body.nextVisibleAt, "nextVisibleAt"),
