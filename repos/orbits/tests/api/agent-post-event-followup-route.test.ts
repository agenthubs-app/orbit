...
test("duplicate contacts enter merge review without proposing writes", async () => {
  const response = await createPostEventFollowup(
    new Request("http://localhost/api/events/demo-event-1/post-event/followup", {
      body: JSON.stringify({
        contactId: "demo-contact-1",
  ...
  assert.equal(response.status, 201);
  const body = await response.json();
  assert.equal(body.data.run.status, "waiting_for_input");
  assert.equal(body.data.artifact.contactResolution, "merge_review_required");
