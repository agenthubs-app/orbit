...
const EXEMPTIONS: { file: string; line: number; reason: string }[] = [
  ...
  // <button> lines).
  { file: "app/(app)/app/contacts/orbit-real-contacts.tsx", line: 393, reason: "filter chip (suggested-query chip, className=\"chip\") — chips are a distinct pattern from .btn, not migrated per audit guidance" },
  { file: "app/(app)/app/contacts/orbit-real-contacts.tsx", line: 398, reason: "filter chip (pipeline-stage filter)" },
  { file: "app/(app)/app/contacts/orbit-real-contacts.tsx", line: 404, reason: "filter chip (value-type filter)" },
  { file: "app/(app)/app/contacts/orbit-real-contacts.tsx", line: 794, reason: "PickerSlot (filled) — card-style contact picker with a stacked avatar+label column layout; .btn's fixed 44px row height would clip it" },
  { file: "app/(app)/app/contacts/orbit-real-contacts.tsx", line: 799, reason: "PickerSlot (empty/dashed) — same card-style picker as above" },
  { file: "app/(app)/app/contacts/orbit-real-contacts.tsx", line: 850, reason: "className=\"card-hover\" — established sitewide clickable-card pattern (also used in home/agent/events), not part of the .btn system" },
  { file: "app/(app)/app/contacts/orbit-real-contacts.tsx", line: 930, reason: "filter chip (intro status filter, desktop)" },
  { file: "app/(app)/app/contacts/orbit-real-contacts.tsx", line: 954, reason: "filter chip (intro status filter, mobile)" },
  { file: "app/(app)/app/contacts/orbit-real-contacts.tsx", line: 1370, reason: "ScanContent dropzone — large card-style upload control (52px/36px padding, icon + heading + subtext); not a compact action button" },
  ...
  { file: "app/(app)/app/agent/orbit-real-agent.tsx", line: 1165, reason: "composer send button — pre-existing bespoke .orbit-agent-send component (gradient background, own focus-visible CSS); no .btn variant supports a gradient fill" },
  { file: "app/(app)/app/agent/orbit-real-agent.tsx", line: 1774, reason: "className=\"orbit-top-icon-btn\" — a distinct CSS-classed primitive (single current usage) excluded because it is not inline-styled" },
  { file: "app/(app)/app/agent/orbit-real-agent.tsx", line: 1796, reason: "history-sidebar resize handle (role=\"separator\", cursor: col-resize) — a drag handle, not button semantics" },
