...
test("the contacts list keeps one narrow analysis link while desktop secondary views are folded", async (t) => {
  ...
  Object.defineProperty(globalThis, "window", { configurable: true, value: {
    location: { href: "https://orbit.test/app/contacts" }, addEventListener() {}, removeEventListener() {},
  } });
  ...
  try {
    ...
    await act(async () => { root = create(<OrbitRealCardsList viewModel={contactsRouteToOrbitContactsViewModel(route)} />); });
    ...
    const labels = () => links().map((link) =>
      [link, ...link.findAllByType("span")]
        .flatMap((node) => node.children)
        .filter((child): child is string => typeof child === "string")
        .join(""),
    );
    assert.equal(links().length, 1);
    assert.equal(root.root.findAll((node) => node.type === "a" && node.props.href === "/app/contacts/graph").length, 0);
    assert.equal(toggle().props["aria-expanded"], false);
    assert.deepEqual(labels(), ["人脉分析"]);
    act(() => toggle().props.onClick());
    assert.equal(links().length, 2);
    assert.equal(toggle().props["aria-expanded"], true);
    assert.deepEqual(labels(), ["人脉分析", "人脉分析"]);
    act(() => toggle().props.onClick());
    assert.equal(links().length, 1);
    assert.equal(toggle().props["aria-expanded"], false);
    assert.deepEqual(labels(), ["人脉分析"]);
