...
import { applyPersonalSchedulePicker } from "../../app/(app)/app/tasks/personal-schedule-picker-model";
import type { PersonalScheduleContract } from "../../shared/contract/tasks";
import { applyPersonalScheduleDuration, buildPersonalScheduleChange, personalScheduleDraft } from "../../app/(app)/app/tasks/personal-schedule-editor-model";
...
test("Next all-day editor reopens the inclusive last occupied date without changing the contract", () => {
...
test("Next all-day editor maps the last occupied day to an exclusive local midnight", () => {
...
test("Next explicit end before the start is invalid rather than silently advanced", () => {
  const draft = { ...personalScheduleDraft(null, "Asia/Tokyo"), title: "Invalid", allDay: true, startDate: "2026-09-17", endDate: "2026-09-16" };
