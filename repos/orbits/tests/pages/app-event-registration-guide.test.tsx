...
test("registration retains the local-parent Kansai identity, language, venue, and canonical aliases", async () => {
  const Page = (await import("../../app/(app)/app/events/[id]/register/page"))
    .default;
  const html = renderToStaticMarkup(
    await Page({
  ...
  await withOrbitModuleMode("hybrid", async () => {
    const zhHtml = renderToStaticMarkup(
      await Page({ params: Promise.resolve({ id: "event_signup_01" }) }),
    );
    ...
    assert.match(zhHtml, /<h1[^>]*>关西跨境商务对接会<\/h1>/);
    assert.match(zhHtml, /data-registration-stage="interview"/);
    assert.equal(
      /data-reg-option/u.test(zhHtml) ||
        /没有使用替代问题|No substitute question was used/u.test(zhHtml),
      true,
    );
    assert.match(enHtml, /<h1[^>]*>Kansai Cross-Border Business Connect<\/h1>/);
    assert.match(enHtml, />Osaka</);
    assert.match(enHtml, /No substitute question was used/);
    assert.doesNotMatch(enHtml, /关西跨境商务对接会|>大阪</);
  });
  ...
  const upcomingEventByPublicCode = await loadEventForRegistration("EVTSIGNUP01");

  assert.equal(upcomingEvent?.status, "imported");
  assert.equal(upcomingEventByPublicCode?.id, "event_signup_01");
  assert.equal(upcomingEvent?.title, "关西跨境商务对接会");
  assert.equal(upcomingEvent?.sourceMetadata.label, "event-core-postgres");
  assert.ok(Date.parse(upcomingEvent?.endsAt ?? "") > Date.now());
  assert.equal(upcomingEvent?.sourceMetadata.provider, "event-core-postgres");
  assert.equal(upcomingEvent?.liveDatabaseWriteExecuted, false);
  assert.equal(upcomingEvent?.externalNetworkRequested, false);
});
