...
test("contact detail renders a business-card contact without the account email or English placeholders (zh)", async () => {
  ...
  assert.doesNotMatch(html, /<span class="nc-fk">行业<\/span>/, "industry row must be hidden when only the placeholder is present");
  assert.doesNotMatch(html, /<span class="nc-fk">最近互动<\/span>/);
...
test("contact detail renders the same legacy contact in English without leaking the account email", async () => {
  ...
  assert.doesNotMatch(html, /<span class="nc-fk">Industry<\/span>/);
});
