...
`, loader: "tsx", resolveDir: process.cwd() },
  ...
  browser = await chromium.launch({ headless: true, ...(process.env.ORBIT_PICKER_QA_CHROME ? { executablePath: process.env.ORBIT_PICKER_QA_CHROME } : {}) });
});
...
test("stored minute is visible and keyboard cancellation restores trigger focus", async t => {
  const page = await open(t, { kind: "time", draft: "09:37" });
  ...
  assert.equal(await page.evaluate(() => document.activeElement?.getAttribute("aria-label")), "Open");
});
