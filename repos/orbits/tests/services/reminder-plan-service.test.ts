...
test("create maps only the typed target-domain error and preserves all other authorizer failures without writes", async () => {
  ...
  assert.equal((await typedHarness.store.listRecords({ limit: "unbounded", workspaceId: "workspace:authorizer-failure", collectionName: "reminderPlans" })).length, 0);
  ...
  for (const [label, failure] of [
    ...
    assert.equal((await failedHarness.store.listRecords({ limit: "unbounded", workspaceId: "workspace:authorizer-failure", collectionName: "reminderPlans" })).length, 0, `${label} failure must not write a plan`);
  }
});
