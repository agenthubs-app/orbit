...
test("configured pure in-app commands save plan and wake intent atomically, replay safely, and cancel the current generation", async (t) => {
  ...
  try {
    ...
    assert.equal((await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "notificationDeliveries" })).length, 0);
...
test("command serializable retry starts a fresh transaction while deadlock errors fail immediately", async (t) => {
  ...
  try {
    ...
    assert.equal((await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "reminderPlans", userId: deadlockActor })).length, 0);
  } finally {
    await db.close();
  }
});
...
test("create retries task and schedule target reads on 40001 and commits one plan plus one wake intent", async (t) => {
  ...
  try {
    ...
    for (const [index, item] of cases.entries()) {
      ...
      assert.equal((await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "reminderPlans", userId: item.actorId })).length, 1);
...
test("create preserves task and schedule target-read 40P01 and performs no plan or wake write", async (t) => {
  ...
  try {
    ...
    for (const [index, item] of cases.entries()) {
      ...
      assert.equal((await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "reminderPlans", userId: item.actorId })).length, 0);
      assert.equal((await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "canonical_reminder_wakes", userId: item.actorId })).length, 0);
    }
  } finally {
    await db.close();
  }
...
test("create rejects a missing schedule target as a domain error without writing a plan or wake", async (t) => {
  ...
  try {
    ...
    assert.equal(publisherCalls, 0);
    assert.equal((await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "reminderPlans", userId: actorId })).length, 0);
    assert.equal((await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "canonical_reminder_wakes", userId: actorId })).length, 0);
  } finally {
    await db.close();
  }
});
...
test("real task termination accepts its owned tombstone and cancels reminders exactly once", async (t) => {
  ...
  try {
    ...
    assert.equal((await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "task_mutations", userId: actorId })).length, 2, "create and delete each leave one receipt");
...
test("plan write rolls back when the durable wake intent write fails", async (t) => {
  ...
  try {
    ...
    assert.equal((await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "reminderPlans" })).length, 0);
    assert.equal((await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "canonical_reminder_wakes" })).length, 0);
  } finally {
    await db.close();
  }
});
