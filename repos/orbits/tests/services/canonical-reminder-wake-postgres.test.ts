...
test("two repair workers can initialize the durable cursor concurrently and deliver each claimed plan once", async (t) => {
  ...
  try {
    ...
    for (const plan of plans) {
      ...
      const deliveries = await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "notificationDeliveries", userId: plan.accountId });
...
test("in-app preferences and delivery fences use the real LiveRecord shape, including recovery and deleted plans", async (t) => {
  ...
  try {
    ...
    assert.equal((await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "notificationDeliveries", userId: actorId })).length, 2);
...
test("generation-only claim is rejected before SQL and leaves intents and the cursor unchanged", async (t) => {
  ...
  try {
    ...
    const beforeIntents = (await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "canonical_reminder_wakes" }))
    ...
    const afterIntents = (await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "canonical_reminder_wakes" }))
      .map((record) => ({ recordId: record.recordId, payload: record.payload, updatedAt: record.updatedAt }))
      .sort((left, right) => left.recordId.localeCompare(right.recordId));
...
test("expired leases are fenced and reclaimed, old generations stay stale, and the 25x10 continuation is fair", async (t) => {
  ...
  try {
    ...
    const remaining = await db.store.listRecords({ workspaceId: db.workspaceId, collectionName: "canonical_reminder_wakes" });
