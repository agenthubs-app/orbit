...
| 1C | 回归基线整顿 | 定位并处理全量测试失败；明确测试库与产品数据边界 | 进行中，首批 5 项数据库回归已修复；显式隔离库全量运行仍有 46 项失败 |
...
| Node 22 显式隔离库全量测试 | 2,238 项：2,191 通过，46 失败，1 跳过；模式与配置前提不同，不能直接比较失败数 |
| 全量 TypeScript | 109 项错误，全部位于测试代码；低于已有 110 项上限，但仍需逐步清理 |
