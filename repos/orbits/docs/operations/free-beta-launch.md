...
| 1C | 回归基线整顿 | 定位并处理全量测试失败；明确测试库与产品数据边界 | 进行中，测试类型与纯类型契约问题已修复；最新显式隔离库全量运行仍有 21 项失败 |
...
| Node 22 类型清理后显式隔离库全量测试 | 2,238 项：2,215 通过，22 失败，1 跳过；与第三批基线一致 |
| Node 22 字典边界修复后显式隔离库全量测试 | 2,238 项：2,216 通过，21 失败，1 跳过；纯类型契约失败已修复，其余不变 |
| 全量 TypeScript | 后端与 iOS 均为 0 项错误；测试错误上限从 110 降为 0 |
| TypeScript ratchet 与语法检查 | 2/2 通过，确认完整项目零类型错误且测试语法无错误 |
| iOS 契约、API Schema 与字典副本检查 | 6/6 通过，没有修改共享响应字段 |
...
- 修复全部 103 项测试类型错误：使用所属领域的正式 DTO、provider 与真实导入模块类型；补齐演进后的样本字段；保留可选 actor 信息和联合类型收窄；环境变量测试完整恢复属性描述符。
- 保留负向行为与断言。缺失证据用例仍使用空证据图和悬空证据 ID，并补充完整来源推断结果断言；没有以删除失败测试或放宽预期值换取通过。
- 全量 TypeScript 在后端和 iOS 均通过。零错误门槛测试 2/2、移动端契约与 Schema 同步检查 4/4 通过；内存中注入不兼容契约后，新增编译期一致性检查按预期报 TS2322，没有修改磁盘源文件。
- Node 22 隔离库全量复跑结果保持 2,215 通过、22 失败、1 跳过，约 81 秒。运行时失败清单与第三批基线一致，没有宣称阶段 1C 通过。
- 三组差异经独立静态复核，未发现遗留问题。变更仅涉及测试和文档，没有数据库迁移、接口字段调整或生产业务修改；行业/语言字典的纯类型目录问题仍待受控同步范围确认。

### 2026-09-06 字典边界修复

- 经确认，将行业和语言运行时字典移至 `shared/domain`，共享契约仅保留类型。所有已知 Web/iOS 消费者改用新入口，业务函数体、14 个行业 ID、42 个三语标签与排序保持不变，未执行数据库迁移。
- 移动端同步脚本只复制两个白名单字典文件，不开放整个 domain 目录。真实命令的临时目录测试覆盖未授权文件排除、过期副本清理和重复同步；连同契约与 Schema 副本检查共 6/6 通过。
- 后端契约与行业测试 6/6、移动端联系人及资源校验相关测试 52/52 通过；两端全量类型检查仍为 0。内存中分别删去一个行业和语言常量，编译期一致性检查均按预期拒绝不完整枚举。
- 独立静态复核未发现问题。隔离库全量复跑为 2,216 通过、21 失败、1 跳过，约 76 秒；只消除了契约目录含运行时代码这一项失败。阶段 1C 仍未通过。
