...
- 顺序：① Network 4 屏 + detail/follow 弹窗 → ② 个人中心（profile/persona/settings；含 onboarding 门禁提示）→ ③ Events 参与者侧（host/live/弹窗；discover/detail 已重建）→ ④ 运营台 7 屏 → ⑤ 认证四态弹窗 → ⑥ iOrbit chat（最后，最大）。
