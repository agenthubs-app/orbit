...
- **验证数据**：沿用 Events 计划的本地库 `orbit_newui_events_20260922`（`.env.local` 已指向；不要重启 dev server）；主办方账号 `organizer@orbit.example.test` / `<password>`；活动 `10000000-0000-4000-8000-000000000001`（a/b 已报名，已交换名片 1 对；生成结果 `not_generated`）。任务 0 由 organizer 在现有运营台「开始生成」并运行一次 `npm run event-operations:worker`（读 `docs/event-operations-e2e.md` 与 `scripts/run-event-operations-worker.ts`），使 `匹配结果`/`两轮分桌` 存在（可不发布，保留「待发布」态给概览/匹配屏）；签到台标记 participant.a 到场一次。
