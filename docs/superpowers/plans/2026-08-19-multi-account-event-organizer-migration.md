...
**Interfaces:**
- Consumes: Task 1 manifest, Task 2 ContactActorLinkProvider, existing `AuthUserService.registerUser()`, AuthUserStorageProvider, and account provisioner.
...
Assert that dry-run creates a stable 13-account/6-link plan without writing; apply requires the reviewed count/hash; missing Xiaoyu identity fails; existing matching emails are reused and re-provisioned; conflicting display name/account chain fails; a second apply reports no new writes and preserves the same final state.
...
--apply --xiaoyu-actor-id <id> --expected-count <n> --expected-plan-hash <64hex>
```
...
```bash
git add repos/orbits/features/events/organizer-accounts/bootstrap.ts repos/orbits/scripts/bootstrap-event-organizer-accounts.ts repos/orbits/package.json repos/orbits/tests/services/event-organizer-account-bootstrap.test.ts repos/orbits/tests/services/event-organizer-account-bootstrap-command.test.ts
...
Run read-only queries that join `auth_users`, `accounts`, and `profiles`; require exactly one complete identity whose profile display name is `小雨`. Stop visibly on zero or multiple matches.
...
- [ ] **Step 3: Apply schema and account-chain repair**

Run:

```bash
npm run db:migrate:live
npm run db:backfill:auth-accounts
```

Verify Event Operations schema version 15 exists and every existing AuthUser has an Account/Profile.

- [ ] **Step 4: Dry-run and apply organizer account bootstrap**

Run dry-run, capture its count/hash, review that it contains 13 account and 6 contact-link operations, then apply with the same count/hash and `ORBIT_DEMO_ORGANIZER_PASSWORD` present only in the process environment.

- [ ] **Step 5: Dry-run and apply 16 legacy owner assignments**

Run dry-run, verify exactly 16 event IDs and the reviewed owner grouping, then apply with expected count/hash.

- [ ] **Step 6: Dry-run and apply standard Event Core backfill**
...
Run read-only SQL assertions for 14 complete organizer identity chains, 16 exact owner assignments, 3 Xiaoyu-owned events, 6 active Xiaoyu ContactActorLinks, 7 non-network owners with no Xiaoyu link, and no Contact/Profile/email-shaped `organizer_actor_id` values.
