...
Assert that dry-run creates a stable 13-account/6-link/1-Xiaoyu-membership/2-canonical-owner-repair/6-contact-owner-repair plan without writing; apply requires the reviewed count/hash; missing or non-Google `agenthubs` identity fails; a Xiaoyu binding that does not resolve to `account_orbit_generated` and `profile_orbit_generated_operator` fails; existing matching organizer emails are reused and re-provisioned; conflicting display name/account chains fail; a second apply reports no new writes and preserves the same final state. The reviewed action count is exactly 28: 13 organizer identities, 6 ContactActorLinks, 1 Xiaoyu auth membership, 2 Xiaoyu canonical LiveRecord owner repairs, and 6 reviewed Contact owner repairs.
...
For Xiaoyu, create one deterministic membership Profile whose payload `id` is the selected `agenthubs` AuthUser ID and whose `accountId` is `account_orbit_generated`. Atomically repair only the outer LiveRecord `user_id` on `account_orbit_generated`, `profile_orbit_generated_operator`, and the 6 reviewed Contact records when it is null, preserving their payloads and metadata. Lock all 8 rows with `SELECT ... FOR UPDATE` before validation so replays cannot race a concurrent owner change. Extend `ensureAccountForUser()` so a complete existing membership prevents creation of a shadow Account/Profile. Reject ambiguous, incomplete, or non-null conflicting ownership instead of silently provisioning another owner boundary.
...
--apply --xiaoyu-auth-user-id <id> --expected-count 28 --expected-plan-hash <64hex>
```
